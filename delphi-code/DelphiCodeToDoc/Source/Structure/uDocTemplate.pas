...
uses Windows, SysUtils, Classes, Contnrs, uTAGComment, uOptions, uDocTagCoverage;
...
type
  ...
  public
    ...
    procedure CopyCommentsFrom(const SourceTemplate : TDOCTemplate);
  end;

  /// Base class for TDOCTemplate containers (holds TDOCTemplate objects)
  //### error Add OutputCategory : TOutputCategoryList; const stSectionCaption : string;
  ...
  TDOCParameterList = class(TDOCTemplateList);  /// List of TDOCparameters
...
{*------------------------------------------------------------------------------
  ...
  @return Same value as CompareText method (generic comparison result)
-------------------------------------------------------------------------------}
function DocTemplateListSortCompare(Item1, Item2: Pointer): integer;
begin
...
{*------------------------------------------------------------------------------
  ...
  @param ln Name of the link
  @return A new created TDOCLink object
-------------------------------------------------------------------------------}
...
begin
  ...
  if (FilenameLength > 4) or (FilenameLength < 3) then exit;
...
{*------------------------------------------------------------------------------
  ...
  @param  SourceTemplate  Source template to copy from
-------------------------------------------------------------------------------}
procedure TDOCTemplate.CopyCommentsFrom(const SourceTemplate: TDOCTemplate);
begin
  FTAGHolderList.CopyContent(SourceTemplate.FTAGHolderList);
end;
