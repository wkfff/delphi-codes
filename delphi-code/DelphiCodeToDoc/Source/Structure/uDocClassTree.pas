...
type
  ...
  {*------------------------------------------------------------------------------
    ...
    @param CurrentClassNodeNode   Class node to iterate
  ------------------------------------------------------------------------------*}
  ...
  public
    ...
    procedure ConvertChildsToTreeNode(tn : TObject; pn : TObject);
  end;
  ...
  private
    ...
    FNodeIInterface : TDOCClassNode; /// IInterface node
    FNodeNoParent : TDOCClassNode; /// No parent node (records)
  ...
  public
    ...
    property NodeIInterface : TDOCClassNode read FNodeIInterface;
    property NodeNoParent : TDOCClassNode read FNodeNoParent;
    ...
    procedure Clear;
    procedure ConvertToTreeNodes(TreeNodes : TObject);
...
begin
  FNodeTObject.Clear;
  FNodeIInterface.Clear;
  FNodeNoParent.Clear;
  FNodeOrphanClass.Clear;
end;

{*------------------------------------------------------------------------------
  Convert the tree to a graphical TreeNode
  @param TreeNodes   TreeNodes to convert to
------------------------------------------------------------------------------*}
procedure TDOCClassesTree.ConvertToTreeNodes(TreeNodes: TObject);
var
  tnRoot : TTreeNode;
  tnList : TTreeNodes;
begin
  tnList := TreeNodes as TTreeNodes;
  Assert(tnList <> nil);
  // Clear the List
  tnList.Clear;
  // Add Root project
  tnRoot := tnList.Add(nil, 'ClassTree');
  // Add Units
  FRootNode.ConvertChildsToTreeNode(TreeNodes, tnRoot);
end;
...
begin
  ...
  FNodeTObject := AddChild(nil,'TObject');
  FNodeTObject.DocClassLink := nil;
  FNodeIInterface := AddChild(nil,'IInterface');
  FNodeIInterface.DocClassLink := nil;
  FNodeNoParent := AddChild(nil,'No parent');
  FNodeNoParent.DocClassLink := nil;
  FNodeOrphanClass := AddChild(nil,'#OrphanClass#');
  FNodeOrphanClass.DocClassLink := nil;
end;

/// Destroy a TDOCClassesTree instance
destructor TDOCClassesTree.Destroy;
begin
...
{*------------------------------------------------------------------------------
  ...
  @return Number of classes under the root node
------------------------------------------------------------------------------*}
...
{*------------------------------------------------------------------------------
  Convert all childs to a graphical tree node                                                                              
  @param tn   ParameterDescription
  @param pn   ParameterDescription
  @return ResultDescription
------------------------------------------------------------------------------*}
{ TODO 3 -oTridenT -cEnhancement : Convert to Iterator ? }
procedure TDOCClassNode.ConvertChildsToTreeNode(tn : TObject; pn : TObject);
var
  ...
  ChildNode : TDocClassNode;
begin
  TreeNodes := tn as TTreeNodes;
  for ChildIndex := 0 to Count - 1 do
  begin
    // Search inside child
    ChildNode := Items[ChildIndex] as TDOCClassNode;
    ParentTreeNode := TreeNodes.AddChild(pn as TTreeNode, ChildNode.Name);
    ChildNode.ConvertChildsToTreeNode(tn, ParentTreeNode);
  end;
end;
