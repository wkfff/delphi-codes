...
uses SysUtils, Math, SourceToken;
...
// TridenT
function TParseTreeNode.GetSourceCode: string;
...
begin
  // TridenT
  if assigned(fcChildNodes) then Result := (ChildNodeCount > 0)
  else Result := False;
end;
...
begin
  Result := nil;
  // TridenT - Verify if childnodes exists
  if(assigned(fcChildNodes)) then
  begin
  for liLoop := 0 to ChildNodeCount - 1 do
    begin
