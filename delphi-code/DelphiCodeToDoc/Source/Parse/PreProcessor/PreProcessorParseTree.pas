...
begin
  ...
  // TridenT - import user settings
  fcDefinedSymbols.Assign(DocProject.DocOptions.ConditionalDefines);
end;
...
begin
  ...
  if fbPreprocessorIncluded then
   // TridenT - Use of DocOptions instead of PreProcessor.OptionIsDefined
    lbEval := DocProject.DocOptions.ConditionalDefines.IndexOf(psOption) >= 0
else
    lbEval := False;
