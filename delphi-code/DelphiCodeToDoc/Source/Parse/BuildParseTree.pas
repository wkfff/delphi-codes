...
type
  ...
  Private
    ...
    // TridenT
    fbInsideCpdStatmt: integer;    //nCompoundStatement
    ...
    // TridenT
    procedure RecogniseCurlyBlock;
    procedure RecogniseBracketStarBlock;
    procedure RecogniseCommentList;
    procedure RecogniseComment;
    procedure RecogniseTAGComment;
    procedure RecogniseTAGContent(cStyle: TCommentStyle);
    procedure RecogniseSlashLine(EndOfLine: boolean);
    ...
    procedure RecogniseConstSectionBody(const pbNestedInClass: Boolean);
    procedure RecogniseResourcestringSection(const pbNestedInClass: Boolean);
    ...
    // TridenT
    procedure RecogniseClassDirectiveName(const TokenType: TTokenType);
    procedure RecogniseClassType;
    // TridenT
    procedure RecogniseRecordTypeAsClass;
    ...
    // TridenT 
    procedure RecogniseNotSolidTokensAndNotComment;
    ...
    // TridenT
    procedure RecogniseCommentsAndExtension(const EndOfLineComments : Boolean);
...
// TridenT
procedure TBuildParseTree.RecogniseCommentsAndExtension(const EndOfLineComments : Boolean);
begin
  while IsComment(fcTokenList.First) do
  begin
   if IsCurlyComment(fcTokenList.First) then RecogniseCurlyBlock()
    else if IsBracketStarComment(fcTokenList.First) then RecogniseBracketStarBlock()
     else if IsSlashComment(fcTokenList.First) then RecogniseSlashLine(EndOfLineComments)
      else Recognise(ttComment);
  end;
end;
...
procedure TBuildParseTree.Recognise(const peTokenTypes: TTokenTypeSet;
  ...
  /// TridenT - Which token can have TAG comment attached ?
  function CanFollowTAGComment(const pts: TTokenTypeSet): boolean;
  begin
    // if inside Block TAG comment are welcome !
    if fbInsideCpdStatmt>0 then Result:=(pts * [ttComment])<>[ttComment]
    //Result:=(pts * [ttComment])<>[ttComment]
    else Result := (pts * (ProcedureWords + [ttUnit,ttLibrary, ttProgram, ttProperty, ttIdentifier, ttClass])) <> [];
  end;
...
begin
  ...
  begin
    // TridenT - WARNING! Prevent infinite loop, ttComment not admite in this while !
    if CanFollowTAGComment(peTokenTypes) then RecogniseCommentsAndExtension(False);
  ...
  // TridenT - After SemiColon, recognise also double slash on same line
  if (ttSemiColon in peTokenTypes) then
  // strap all Token in NotSolidTokensOnLine set
   while (fcTokenList.First.TokenType in NotSolidTokensOnLine) do
   begin
     if fcTokenList.First.TokenType<>ttComment then
      Recognise(fcTokenList.First.TokenType)
     else
      RecogniseCommentsAndExtension(True);
   end;
  Inc(fiTokenCount);
  // TridenT - No need here
...
begin
  ...
  // TridenT - Not solid token already recognized
  //RecogniseNotSolidTokens;
...
begin
  ...
  begin
    Recognise(ttIn);
    // TridenT - UsesFileName have a special node
    PushNode(nUsesFilename);
...
begin
  // TridenT - Added node for Dotted name
  PushNode(nDottedIdentifier);
  RecogniseIdentifier(False, idStrict);
...
begin
  ...
  case lt of
    ...
    ttResourceString:
      RecogniseResourcestringSection(false);
    ttType:
...
// #TridenT : Const section body used by RecogniseConstSection() and RecogniseResourcestringSection()
procedure TBuildParseTree.RecogniseConstSectionBody(const pbNestedInClass: Boolean);
var
...
begin
  ...
  Recognise(ttConst);
  // TridenT
  RecogniseConstSectionBody(pbNestedInClass);
  PopNode;
end;

// TridenT
procedure TBuildParseTree.RecogniseResourcestringSection(const pbNestedInClass: Boolean);
begin
  {
    ConstSection -> CONST (ConstantDecl ';')...
  }
  PushNode(nResourcestringSection);
  Recognise(ttResourceString);
  // TridenT
  RecogniseConstSectionBody(pbNestedInClass);
  PopNode;
end;
...
begin
  ...
  // TridenT 
  if (fcTokenList.FirstSolidTokenType in StructuredTypeWords) then
...
begin
  ...
  case lc.TokenType of
    ...
    ttFile:
      RecogniseFileType;
    ttRecord:
      // TridenT - Recognize record as class
      RecogniseRecordTypeAsClass;
    else
...
begin
  ...
  // TridenT - Recognize method already handle non solid tokens !
  //lcNextToken := fcTokenList.FirstSolidToken;
...
var
  lbHasVars: Boolean;
begin
  // TridenT - Included in RecogniseVarDecl()
  //PushNode(nClassVars);
  Recognise(ttClass);
  Recognise(ttVar);
...
begin
  ...
  Recognise(ttBegin);
  // TridenT
  inc(fbInsideCpdStatmt);
  RecogniseStatementList([ttEnd]);
  Recognise(ttEnd);
  // TridenT
  dec(fbInsideCpdStatmt);
  PopNode;
end;
...
begin
  ...
  begin
    // TridenT - Force recognise trailing not solid token
    RecogniseNotSolidTokensAndNotComment();
    // Eventually, recognise Comments just after procedure headings
    RecogniseCommentsAndExtension(False);
    ...
    begin
      ...
      case fcTokenList.FirstSolidTokenType of
        ...
        begin
          ...
          // TridenT - Insert a node to find more easily ProcedureDirectiveName
          PushNode(nProcedureDirectiveName);
...
// TridenT
procedure TBuildParseTree.RecogniseClassDirectiveName(const TokenType: TTokenType);
begin
  if TokenType in ClassDefinitionDirectives then
  begin
    PushNode(nClassDirectiveName);
    Recognise(TokenType);
    PopNode;
  end;
end;
...
begin
  ...
  begin
    // TridenT
    RecogniseClassDirectiveName(ttHelper);
  ...
  begin
    ...
    if fcTokenList.FirstSolidTokenType = ttSealed then
     // TridenT 
     RecogniseClassDirectiveName(ttSealed);

    // abstract class
    if fcTokenList.FirstSolidTokenType = ttAbstract then
      // TridenT
      RecogniseClassDirectiveName(ttAbstract);
...
// TridenT
procedure TBuildParseTree.RecogniseRecordTypeAsClass;
begin
  PushNode(nRecordType);
  ...
  RecogniseClassBody;
  { TO DELETE if fcTokenList.FirstSolidTokenType = ttCase then // it is a variant record
    RecogniseVariantSection; }
  Recognise(ttEnd);
...
begin
  ...
  lbStarted := False;
  // TridenT - Added [ttCase] for variant record support
  while (fcTokenList.FirstSolidTokenType in (CLASS_DECL_WORDS + [ttOpenSquareBracket] + [ttCase])) or
  ...
  begin
    ...
    case lc.TokenType of
      // WAS DELETED IN JCF
      // Added ttCase for variant record support
      
      ttCase: // it is a variant record
      begin
        RecogniseVariantSection;
        //Recognise(ttEnd);
...
// TridenT
procedure TBuildParseTree.RecogniseNotSolidTokensAndNotComment;
begin
  while (fcTokenList.CurrentTokenIndex < fcTokenList.Count) and
    (fcTokenList.FirstTokenType in NotSolidTokensAndNotComment) do
  begin
    TopNode.AddChild(fcTokenList.Extract);
  end;
end;
...
begin
  ...
  // TridenT - Change to recognise only SolidTokens
  // while fcTokenList.FirstTokenType in LiteralStringStarters do
  while fcTokenList.FirstSolidTokenType in LiteralStringStarters do
  begin
    case fcTokenList.FirstSolidTokenType of
      ttQuotedLiteralString:
      begin
      ...
      begin
        ...
        // TridenT - To verify
        if fcTokenList.FirstSolidTokenLength = 1 then
          Recognise(fcTokenList.FirstTokenType)
        else
...
begin
  repeat
    ...
    // TridenT
    //RecogniseNotSolidTokens;
    until fcTokenList.FirstSolidTokenType <> ttOpenSquareBracket;
end;

procedure TBuildParseTree.RecogniseCurlyBlock;
begin
  {CurlyBlock:
   -> OpenCurly
   -> Comments list
  ...
  PushNode(nCurlyBlock);
  Recognise(ttComment);
  RecogniseCommentList;
  Recognise(ttComment);
  PopNode;
end;

procedure TBuildParseTree.RecogniseSlashLine(EndOfLine: boolean);
begin
  {Slash Comment:
   -> DoubleSlash
   -> (1) Comments list
  ...
  if EndOfLine then
  begin
    PushNode(nEndOfLineComment)
  end
  else begin
    PushNode(nInlineComment)
  end;
  // DoubleSlash comment
  Recognise(ttComment);
  if IsStandardComment(TokenList.First) then
  begin
    Recognise(ttComment)
  end
  else  begin
    if IsTAGComment(TokenList.First) then
    begin
      RecogniseTAGComment
    end
    else      begin
  ...
  PopNode; // nInlineComment
end;

procedure TBuildParseTree.RecogniseCommentList;
begin
  {CommentList:
   -> [Comment | TAGComment] ...
   ClosedCommentedBlock
  }
  PushNode(nCommentList);

  while not (IsBlockComment(TokenList.First)) do
  begin
    if IsTAGComment(TokenList.First) then
    begin
      RecogniseTAGComment
...
procedure TBuildParseTree.RecogniseComment;
begin
  // Comment -> comment, comment, comment ... ]
  PushNode(nTAGComment);
  // ClosedCurly ?
  if IsCurlyComment(TokenList.First) then
  begin
    PopNode;
    Exit;
  end;
  // No TAG specifier
  // Recognise TAG Content
  while (TokenList.First.CommentStyle in [eContentTAG,eReturn]) do
  begin
    // detect in-comment return or content
    if TokenList.First.CommentStyle=eReturn then Recognise(ttComment)
     // Detect Inline Extended description
     else if fbInsideCpdStatmt>0 then RecogniseTAGContent(eAdditionalCommentTAG)
      else RecogniseTAGContent(eDescriptionTAG);
  end;
  PopNode;
end;

procedure TBuildParseTree.RecogniseTAGComment;
var
  lc: TSourceToken;
begin
  // Comment -> TAG_Specifier [TAG Content]
  PushNode(nTAGComment);
  // ClosedCurly ?
  if IsCurlyComment(TokenList.First) then
  begin
  ...
  lc := TokenList.First;
  // Recognise TAG specifier
  Recognise(ttComment);
  // Recognize Param TAG identifier after @Param TAG
  if lc.CommentStyle = eParamTAG then
   if TokenList.First.CommentStyle = eParameterIdentifier then Recognise(ttComment);
  ...
  begin
    // detect in-comment return or content
    if TokenList.First.CommentStyle=eReturn then Recognise(ttComment)
    else RecogniseTAGContent(lc.CommentStyle);
  end;
  PopNode;
end;

procedure TBuildParseTree.RecogniseTAGContent(cStyle: TCommentStyle);
var
  ptNodeType: TParseTreeNodeType;
begin

  case cStyle of
    eUnknownTAG:
    begin
      ptNodeType := nTAGUnknown
    end;
    eDescriptionTAG:
    begin
      // Description TAG in a Block procedure or function is in fact
      //  an Extended comment, so convert to @AdditionalComment !
      ptNodeType := nTAGDescription
    end;
    eAuthorTAG:
    begin
      ptNodeType := nTAGAuthor
    end;
    eVersionTAG:
    begin
      ptNodeType := nTAGVersion
    end;
    eTodoTAG:
    begin
      ptNodeType := nTAGTodo
    ...
    eParamTAG:
    begin
      ptNodeType := nTAGParam
    end;
    eReturnTAG:
    begin
      ptNodeType := nTAGReturn
    end;
    eSeeTAG:
    begin
      ptNodeType := nTAGSee
    ...
    eThrowsTAG:
    begin
      ptNodeType := nTAGThrows
    end;
    eAdditionalCommentTAG:
    begin
      ptNodeType := nTAGAdditionalComment
    end;
    else
    begin
      ptNodeType := nTAGUnknown
  ...
  // TAGContent -> one line of comment
  PushNode(ptNodeType);
  // Recognise content
  Recognise(ttComment);
  PopNode;
...
procedure TBuildParseTree.RecogniseBracketStarBlock;
begin
  {BracketStarBlock:
   -> OpenBracketStar
   -> Comments list
