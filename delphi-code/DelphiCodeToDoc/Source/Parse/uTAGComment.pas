...
uses
  Classes, SysUtils, Tokens, Contnrs, ParseTreeNodeType, Generics.Collections;

type
  TDOCTag = (dtDescriptionTAG, dtAuthorTAG, dtVersionTAG
  ...
  TSubTAGList = class(TObjectList<TSubTAG>)
  public
  ...
  TTAGPatternList = class(TObjectList<TTAGPattern>)
  public
  ...
  private
    ...
    function NewTAG(const TAGstyle: TCommentStyle): TTAGHolderListItem; overload;
  ...
  public
    ...
    procedure CopyContent(const SourceList : TTAGHolderList);
  end;
...
{*------------------------------------------------------------------------------
  Create a new TAG (from TTAGPattern) and add it to the list
  @param  TAGType Tag type to add
  @return Item created in the list
-------------------------------------------------------------------------------}
...
{*------------------------------------------------------------------------------
  Create a new TAG (from TCommentStyle) and add it to the list
  @param  TAGstyle Tag type to add
  @return Item created in the list
-------------------------------------------------------------------------------}
function TTAGHolderList.NewTAG(const TAGstyle: TCommentStyle): TTAGHolderListItem;
begin
  Result := TTAGHolderListItem.Create(TAGPatternList.FindItemByStyle(TAGstyle));
  Add(Result);
end;
...
{*------------------------------------------------------------------------------
  Create a new TAG (from TParseTreeNodeType) and add it to the list
  @param  NodeType Tag type to add
  @return Item created in the list
-------------------------------------------------------------------------------}
...
// Copy content of source tags to the caller
procedure TTAGHolderList.CopyContent(const SourceList: TTAGHolderList);
var
  IndexItem : integer;
  ListItem : TTAGHolderListItem;
  IndexSubId: Integer;
  IndexLine : Integer;
  SubTag : TSubTAG;
begin
  //
  for IndexItem := 0 to SourceList.Count - 1 do
  begin
    ListItem := SourceList[IndexItem];
    for IndexSubId := 0 to ListItem.SubTAGList.Count - 1 do
    begin
      SubTAG := ListItem.SubTAGList[IndexSubId];
      for IndexLine := 0 to SubTAG.FTAGContent.Count - 1 do
       AddTAG(SubTAG.FTAGContent[IndexLine], ListItem.TAGItem.FTNType
        , SubTAG.FTAGContentID);
    end;
  end;      
end;
...
procedure RegisterTAG(ALanguage: Boolean);
begin
  if ALanguage then
  begin
    ...
    TAGPatternList.RegisterTAG('功能', eDescriptionTAG, nTAGDescription);
    TAGPatternList.RegisterTAG('@UNKNOWN', eUnknownTAG, nTAGUnknown);
    TAGPatternList.RegisterTAG('作者', eAuthorTAG, nTAGAuthor);
    TAGPatternList.RegisterTAG('版本', eVersionTAG, nTAGVersion);
    TAGPatternList.RegisterTAG('参考', eSeeTAG, nTAGSee);
    TAGPatternList.RegisterTAG('参数', eParamTAG, nTAGParam);
    TAGPatternList.RegisterTAG('返回', eReturnTAG, nTAGReturn);
    TAGPatternList.RegisterTAG('@TODO', eTodoTAG, nTAGTodo);
    TAGPatternList.RegisterTAG('@THROWS', eThrowsTAG, nTAGThrows);
    TAGPatternList.RegisterTAG('@COMMENT', eAdditionalCommentTAG, nTAGAdditionalComment);
  end;
