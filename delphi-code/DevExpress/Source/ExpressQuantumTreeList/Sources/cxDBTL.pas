...
uses
  Windows, SysUtils, Classes, Variants, cxVariants, DB, cxEdit,
...
type
  ...
  protected
    ...
    property DataSet: TDataSet read GetDataSet;
    property DataSetLayoutChanged: Boolean read FDataLayoutChanged write FDataLayoutChanged;
...
destructor TcxTreeListDataLink.Destroy;
begin
  DataLinks.Remove(Self);
  inherited Destroy;
end;
...
function TcxTreeListDataLink.IsDataSetBusy: Boolean;
var
  I: Integer;
  ADataLink: TcxTreeListDataLink;
begin
  Result := False;
  for I := 0 to DataLinks.Count - 1 do
  begin
    ADataLink := TcxTreeListDataLink(DataLinks[I]);
    Result := (ADataLink.DataSet = DataSet) and ADataLink.DataBusy;
...
begin
  if TreeList.DataSetLayoutChanged then Exit;
  TreeList.DataSetLayoutChanged := True;
  try
    DataSetChanged;
  finally
    TreeList.DataSetLayoutChanged := False;
  end;
end;
...
begin
  ...
  begin
    ...
    AParentNode := TreeList.FindNodeByKeyValue(TcxDBTreeListNode(TreeList.FocusedNode).ParentKeyValue);
...
begin
  inherited Cancel;
  if (DataSet <> nil) and not UpdateEditing then
    DataSet.Cancel;
end;
...
begin
  if (FocusedNode <> nil) and VarEquals(TcxDBTreeListNode(FocusedNode).KeyValue, AKeyValue) then
   ...
   begin
     ...
     if (Result = nil) and (UpdatingNode = nil) and not DataBusy and not FIsRefresh then
     begin
       FIsRefresh := True;
       try
         Loader.Load(nil);
