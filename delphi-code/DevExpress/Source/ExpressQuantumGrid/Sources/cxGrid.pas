...
uses
  ...
  dxCore, dxMessages, cxClasses, cxGraphics, cxLookAndFeels, cxLookAndFeelPainters,
  cxControls, cxPC, cxNavigator, cxCustomData,
...
type
  ...
  TcxGridPrepareLockedImageEvent = procedure(Sender: TcxCustomGrid; AImage: TcxBitmap32; var ADone: Boolean) of object;

  TcxCustomGrid = class(TcxControl, IcxNavigator, IcxNavigatorRecordPosition, IdxSkinSupport)
  private
    ...
    FOnPrepareLockedImage: TcxGridPrepareLockedImageEvent;
  ...
  {$ENDIF}
    ...
    function CanCreateLockedImage: Boolean;
    procedure CreateLockedImage;
    procedure DrawLockedImage;
    function DoLockedImagePrepare: Boolean; virtual;
    procedure PrepareLockedImage;
    procedure ReleaseLockedImage;
    ...
    property IsLockedImageReady: Boolean read FIsLockedImageReady;
    property IsPopupControl: Boolean read FIsPopupControl write FIsPopupControl;
    property LockedImage: TcxBitmap32 read FLockedImage;
    property NavigatorNotifier: TcxNavigatorControlNotifier read FNavigatorNotifier;
    property OnPrepareLockedImage: TcxGridPrepareLockedImageEvent read FOnPrepareLockedImage write FOnPrepareLockedImage;
  public
  ...
  {$ENDIF}
    ...
    procedure BeginUpdate{(ACreateLockedImage: Boolean = False)};
...
uses
  SysUtils, Math, cxGridCustomTableView, dxOffice11, cxGeometry;
...
begin
  if Control.IsLockedImageReady then
    Result := Control.LockedImage.cxCanvas
  else
...
begin
  inherited;
  if UpdateLocked and (LockedImage <> nil) then
    DrawLockedImage
  else
    FPainter.Paint;
end;
...
function TcxCustomGrid.CanCreateLockedImage: Boolean;
begin
  Result := not IsUpdating and (FUpdateLockCount = 0) and HandleAllocated and
     ([csLoading, csDestroying] * ComponentState = []);
end;

procedure TcxCustomGrid.CreateLockedImage;
begin
  ReleaseLockedImage;
  FLockedImage := TcxBitmap32.CreateSize(BoundsRect);
  cxPaintTo(Self, FLockedImage.cxCanvas, Point(0, 0), Rect(0, 0, Width, Height));
end;

procedure TcxCustomGrid.DrawLockedImage;
begin
  if not FIsLockedImageReady then
    PrepareLockedImage;
  ActiveCanvas.Canvas.StretchDraw(Rect(0, 0, Width, Height), LockedImage);
end;

function TcxCustomGrid.DoLockedImagePrepare: Boolean;
begin
  Result := False;
  if Assigned(FOnPrepareLockedImage) then
    FOnPrepareLockedImage(Self, LockedImage, Result);
end;

procedure TcxCustomGrid.PrepareLockedImage;
{
var
  W, H: Integer;
  R: TRect;
...
begin
  try
    if not DoLockedImagePrepare then
    begin
      LockedImage.Lighten(40);
      //!!!TODO: out waiting message
      {
      W := 100;
      H := 30;
      R := cxRectCenter(ClientBounds, W, H);
      LockedImage.cxCanvas.FillRect(R, clRed);
      }
    end;
  finally
    FIsLockedImageReady := True;
    Changed(TcxGridLayoutChange.Create(nil));
  end;
end;

procedure TcxCustomGrid.ReleaseLockedImage;
begin
  if FLockedImage = nil then Exit;

  FreeAndNil(FLockedImage);
  FIsLockedImageReady := False;
end;
...
procedure TcxCustomGrid.BeginUpdate{(ACreateLockedImage: Boolean = False)};
begin
  {
  if ACreateLockedImage and CanCreateLockedImage then
    CreateLockedImage;
  }
