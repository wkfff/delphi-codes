...
uses
  ...
  cxStyles, cxStorage, cxGridCommon, cxCustomData, cxData, cxListBox, cxPC, ImgList;
...
{$ENDIF}
  ...
  protected
    ...
    procedure RestoreFrom(const AStorageName: string; AStream: TStream;
      ...
      AOptions: TcxGridStorageOptions; const ARestoreViewName: string); virtual;
...
begin
  FOffset := Canvas.TextHeight(dxMeasurePattern) div 6;
end;
...
begin
  Result := Canvas.TextHeight(dxMeasurePattern) + 2 * FItemTextOffsetVert;
  if ShowChecks then
...
begin
  ...
  begin
    ...
    begin
      if (FPrevHintText = AText) and EqualRect(FHintTextRect, ATextRect) then
        Exit;
      EndMouseTracking(Self);
    end;
  FPrevHintText := AText;
  ...
  AHintWindowRect := GetHintWindowRect;
  if not cxProcessControlHintInfo(AHintInfo, FHintWindow, Control, AText,
    AHintWindowRect, AHintCellViewInfo) then Exit;
  ...
  begin
    R := AHintAreaBounds;
    with Site.ClientToScreen(cxNullPoint) do
      OffsetRect(R, X, Y);
    AIntf.SetHintAreaBounds(R);
  end;
...
begin
  if Infos[ALevel].SummaryItemIndex <> -1 then
    QuickSortList(ANode.FChilds.List, ANode.FChilds.Count, CompareGroupsBySummary);
...
begin
  ...
  if not Result and not ((ssRight in Shift) or cxShiftStateMoveOnly(Shift) and MouseRightButtonReleased) and
...
var
  R: TRect;
begin
  R := cxRectSetLeft(BoundsRect, cxGridInvisibleCoordinate);
  MoveWindow(R);
end;
  ...
  begin
    with ANewBounds do
      Windows.MoveWindow(Handle, Left, Top, Right - Left, Bottom - Top,
        ANewBounds.Left <> cxGridInvisibleCoordinate);
  end;
end;
...
begin
  if Left = cxGridInvisibleCoordinate then Exit;
  if not GridView.IsUpdateLocked and not FViewInfo.IsCalculating then
  begin
    inherited Paint;
    Painter.Paint;
  end
  ...
  begin
    ...
    if AGrid.IsLockedImageReady then
      cxBitBlt(ActiveCanvas.Handle, AGrid.LockedImage.cxCanvas.Handle,
        Rect(0,0, Width, Height), BoundsRect.TopLeft, SRCCOPY);
  end;
end;
...
begin
  Canvas.Font := AFont;
  Result := Canvas.TextHeight(dxMeasurePattern);
end;
...
procedure TcxCustomGridView.BeginUpdate{(ACreateLockedImage: Boolean = False)};
begin
  if Control <> nil then
    TcxCustomGrid(Control).BeginUpdate{(ACreateLockedImage)};
...
begin
  RestoreFrom(AStorageName, nil, TcxIniFileReader, AChildrenCreating,
    AChildrenDeleting, AOptions, ARestoreViewName);
end;
...
procedure TcxCustomGridView.StoreToStream(AStream: TStream; AOptions: TcxGridStorageOptions;
  const ASaveViewName: string);
begin
  StoreTo('', AStream, TcxStreamWriter, True, AOptions, ASaveViewName);
end;
