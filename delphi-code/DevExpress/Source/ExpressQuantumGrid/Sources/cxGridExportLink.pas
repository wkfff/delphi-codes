...
uses
  dxXLSXExport, cxXLSExport, cxHtmlXmlTxtExport, Math, Dialogs,
...
begin
  ...
  try
    ...
    begin
      ...
      Provider.SetCellPNGImage(AColumn, ARow,
        GraphicToPNGImage(AClientRect, R, APicture.Graphic, True));
    end;
  finally
    APicture.Free;
  end;
...
begin
  ...
  AViewInfo := TDataRowAccess(ARow).GetViewInfoClass.Create(
    ViewInfo.RecordsViewInfo, ARow) as TcxGridDataRowViewInfo;
  try
    AViewInfo.Recalculate;
    RecordHeight := AViewInfo.ContentHeight;
    if AViewInfo.HasPreview then
      Dec(FRecordHeight, AViewInfo.PreviewViewInfo.Height);
    ALeft := AddIndents(ATop, AViewInfo.ContentHeight, ARow, ARow.Expandable);
    SetPatternsHeight(RecordHeight);
