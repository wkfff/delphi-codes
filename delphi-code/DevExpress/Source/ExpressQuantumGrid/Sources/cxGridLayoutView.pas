...
type
  ...
  protected
    ...
    property LoadedGridViewItem: string read FLoadedGridViewItem;
  public
...

{
  ...
  protected
    ...
    procedure IdxLayoutLookAndFeelUser.LookAndFeelChanged = LayoutLookAndFeelChanged;
    procedure LayoutLookAndFeelChanged; stdcall;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        AGridViewItem := TcxGridLayoutViewItem(GridView.FindItemByObjectName(
          TcxGridLayoutItem(AItem).LoadedGridViewItem));
        if AGridViewItem <> nil then
          TcxGridLayoutItem(AItem).GridViewItem := AGridViewItem
        else
          AItem.Free;
      end;
    end;
...
procedure TcxGridLayoutContainer.BeforeRestoring;
begin
  IsRestoring := True;
end;
...
begin
  with ViewInfo do
    LookAndFeelPainter.LayoutViewDrawRecordExpandButton(Canvas,
      Bounds, ButtonState, RecordViewInfo.Expanded);
end;
...
begin
  with ViewInfo do
    LookAndFeelPainter.LayoutViewDrawRecordCaption(Canvas, Bounds,
      RecordViewInfo.RecordCaptionPosition, GetState);
end;
...
begin
  SaveCloneData;
  R := cxRectOffset(LayoutViewInfo.ContentBounds, LayoutViewInfo.ContentBounds.TopLeft, False);
  CalculateLayoutViewInfo;
  ANeedRefresh := not EqualRect(R, cxRectOffset(LayoutViewInfo.ContentBounds, LayoutViewInfo.ContentBounds.TopLeft, False));
...
begin
  Result := GridRecord.CloneDataStream.Position > 0;
  if Result then
  begin
...
begin
  ...
  try
    GridView.StylesAdapter.GetItemParams(GridRecord, nil, AParams);
    PrepareCaptionOptions(GridView.LayoutLookAndFeel.ItemOptions.CaptionOptions, AParams);
    GridView.StylesAdapter.GetGroupParams(GridRecord, nil, AParams);
    PrepareCaptionOptions(GridView.LayoutLookAndFeel.GroupOptions.CaptionOptions, AParams);
    GridView.LayoutLookAndFeel.GroupOptions.Color := AParams.Color;
  finally
...
begin
  CalculateLayout;
  if not GridRecord.Expanded and IsRecordCaptionRotated then
    Result := GetRecordCaptionHeight
  else
...
begin
  Result := GridViewInfo.CalculateDown and (VisibleCount > 0) and GridView.OptionsView.CenterRecords and not GridViewInfo.IsHeightCalculating;
end;
...
begin
  for I := 0 to ViewInfo.RecordsViewInfo.Count - 1 do
    ViewInfo.RecordsViewInfo[I].ResetCloneData;
end;
...
begin
  Result := cxRectInflate(ViewInfo.Bounds, -OptionsView.RecordBorderWidth, -OptionsView.RecordBorderWidth);
  Result := cxRectInflate(Result, -OptionsView.RecordIndent, -OptionsView.RecordIndent); 
...
procedure TcxGridLayoutView.BeginLookAndFeelDestroying;
begin
// do nothing
end;

procedure TcxGridLayoutView.EndLookAndFeelDestroying;
begin
// do nothing
end;

procedure TcxGridLayoutView.LayoutLookAndFeelChanged;
begin
  Container.BeginUpdate;
  try
    TdxCustomLayoutItemAccess(Root).LayoutLookAndFeelUserChanged;
  finally
    Container.EndUpdate(False);
  end;
end;

procedure TcxGridLayoutView.LookAndFeelDestroyed;
begin
// do nothing
end;
...
begin
  ...
  FContainer.HighlightRoot := False;
