...
type
  ...
  TcxGridEditAutoHeight = (eahNone, eahEditor, eahRow);
...
//    property Owner: IcxGridFilterPopupOwner read GetOwnerValue write SetOwnerValue;
  ...
  TcxGridMultilineEdit = class;

  TcxGridMultilineEditViewData = class(TcxCustomMemoViewData)
  private
    function GetEdit: TcxGridMultilineEdit;
  public
    function GetClientExtent(ACanvas: TcxCanvas;
      AViewInfo: TcxCustomEditViewInfo): TRect; override;
    property Edit: TcxGridMultilineEdit read GetEdit;
  end;

  TcxGridMultilineEditViewInfo = class(TcxCustomMemoViewInfo)
  private
    function GetEdit: TcxGridMultilineEdit;
  protected
  ...
  TcxGridMultilineEditProperties = class(TcxMemoProperties)
  public
    class function GetViewDataClass: TcxCustomEditViewDataClass; override;
    class function GetViewInfoClass: TcxContainerViewInfoClass; override;
  end;

  TcxGridMultilineEdit = class(TcxMemo)
  private
    FCalculatedHeight: Integer;
    FAutoHeightMode: TcxGridEditAutoHeight;
    FEditingController: TcxGridEditingController;
    procedure WMCommand(var Message: TWMCommand); message WM_COMMAND;
  protected
    procedure CalculateHeight;

    property AutoHeightMode: TcxGridEditAutoHeight read FAutoHeightMode write FAutoHeightMode;
  ...
  public
    constructor Create(AEditingController: TcxGridEditingController); reintroduce; virtual;
  ...
  protected
    procedure AdjustMultilineEdit; virtual;
    ...
    procedure CloseEdit;
    function CreateMultilineEdit: TcxGridMultilineEdit; virtual;
    ...
    procedure UpdateEditHeight(AHeight: Integer);
    ...
    property MultiLineEdit: TcxGridMultilineEdit read FMultiLineEdit;
  public
  ...
  TcxGridItemEditAutoHeight = (ieahDefault, iaehNone);
  ...
  private
    FEditAutoHeight: TcxGridItemEditAutoHeight;
    FEditing: Boolean;
    FFiltering: Boolean;
    ...
    FSorting: Boolean;
    procedure SetEditAutoHeight(Value: TcxGridItemEditAutoHeight);
  ...
  protected
    ...
    property EditAutoHeight: TcxGridItemEditAutoHeight read FEditAutoHeight
      write SetEditAutoHeight default ieahDefault;
  ...
  protected
    ...
    function GetEditAutoHeight: TcxGridEditAutoHeight; virtual;
  ...
  private
    ...
    FEditAutoHeight: TcxGridEditAutoHeight;
    ...
    procedure SetEditAutoHeight(Value: TcxGridEditAutoHeight);
  ...
  protected
    property EditAutoHeight: TcxGridEditAutoHeight read FEditAutoHeight write SetEditAutoHeight default eahNone;
...
{$ENDIF}
  ...
  cxGrid, cxGridStrs, cxGridLevel, cxLibraryConsts, dxCore, cxGeometry;
...
{ TcxGridMultilineEditViewData }

function TcxGridMultilineEditViewData.GetClientExtent(ACanvas: TcxCanvas;
  AViewInfo: TcxCustomEditViewInfo): TRect;
begin
  if Edit.AutoHeightMode = eahEditor then
    Result := cxSimpleRect
  else
    Result := inherited GetClientExtent(ACanvas, AViewInfo);
end;

function TcxGridMultilineEditViewData.GetEdit: TcxGridMultilineEdit;
begin
  Result := TcxGridMultilineEdit(inherited Edit);
end;

{ TcxGridMultilineEditViewInfo }

function TcxGridMultilineEditViewInfo.GetEdit: TcxGridMultilineEdit;
begin
  Result := TcxGridMultilineEdit(inherited Edit);
end;

procedure TcxGridMultilineEditViewInfo.InternalPaint(ACanvas: TcxCanvas);
var
  AGridItem: TcxCustomGridTableItem;
  ABorders: TcxBorders;
  ABorderColor: TColor;
  AOptionsView: TcxCustomGridTableOptionsView;
begin
  if Edit.AutoHeightMode = eahEditor then
  begin
    AOptionsView := Edit.EditingController.GridView.OptionsView;
    ABorderColor := AOptionsView.EditAutoHeightBorderColor;
    if (ABorderColor <> clNone) and Edit.ModifiedAfterEnter then
    begin
      if ABorderColor = clDefault then
        ABorderColor := AOptionsView.GetGridLineColor;
      ACanvas.FrameRect(Bounds, ABorderColor, 1, cxBordersAll, True)
    end
    else
    begin
      ABorders := [bTop, bBottom];
      AGridItem := Edit.EditingController.EditingItem;
      if AGridItem.IsFirst then
        Include(ABorders, bLeft);
      if AGridItem.IsLast then
        Include(ABorders, bRight);
      ACanvas.DrawFocusRect(Bounds);
      ACanvas.ExcludeFrameRect(Bounds, 1, ABorders);
    end;
  end;
  inherited InternalPaint(ACanvas);
end;

{ TcxGridMultilineEditProperties }

class function TcxGridMultilineEditProperties.GetViewDataClass: TcxCustomEditViewDataClass;
begin
  Result := TcxGridMultilineEditViewData;
end;

class function TcxGridMultilineEditProperties.GetViewInfoClass: TcxContainerViewInfoClass;
begin
  Result := TcxGridMultilineEditViewInfo;
end;

{ TcxGridMultilineEdit }

constructor TcxGridMultilineEdit.Create(
  AEditingController: TcxGridEditingController);
begin
  inherited Create(nil, True);
  FEditingController := AEditingController;
end;

class function TcxGridMultilineEdit.GetPropertiesClass: TcxCustomEditPropertiesClass;
begin
  Result := TcxGridMultilineEditProperties;
end;

procedure TcxGridMultilineEdit.CalculateHeight;
var
  R: TRect;
  DC: HDC;
  H: Integer;
...
begin
  ...
  InflateRect(R, -1, 0);
  DC := GetDC(0);
  SelectObject(DC, Style.Font.Handle);
  H := DrawText(DC, PChar(S), Length(S), R, DT_EDITCONTROL or DT_WORDBREAK or DT_CALCRECT or DT_NOPREFIX);
  ReleaseDC(0, DC);
  Inc(H, 4);
  FCalculatedHeight := H;
end;

procedure TcxGridMultilineEdit.WMCommand(var Message: TWMCommand);
begin
  if (Message.NotifyCode = EN_UPDATE) and InnerControl.HandleAllocated and
    (Message.Ctl = InnerControl.Handle) then
  begin
    CalculateHeight;
    EditingController.UpdateEditHeight(CalculatedHeight);
  end;
  inherited;
end;
...
procedure TcxGridEditingController.AdjustMultilineEdit;
begin
  MultiLineEdit.AutoHeightMode := EditingItem.GetEditAutoHeight;
  MultiLineEdit.Properties.WantReturns := False;
end;
...
begin
  FIsEditAutoHeight := EditingItem.CanEditAutoHeight;
  if not FIsEditAutoHeight then Exit;
  FMultiLineEdit.ActiveProperties.LockUpdate(True);
  try
    FMultiLineEdit.ActiveProperties.Assign(FEdit.ActiveProperties);
    AdjustMultilineEdit;
  finally
    FMultiLineEdit.ActiveProperties.LockUpdate(False);
  end;
...
function TcxGridEditingController.CreateMultilineEdit: TcxGridMultilineEdit;
begin
  Result := TcxGridMultilineEdit.Create(Self);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FEditPlaceBounds := EditBounds;
        if EditingItem.GetEditAutoHeight = eahEditor then
        begin
          InflateRect(FEditPlaceBounds, 1, 1);
          FInitialEditHeight := FEditPlaceBounds.Bottom - FEditPlaceBounds.Top;
        end;
      end
      else
      begin
        ...
        GridView.DoUpdateEdit(FEditingItem, FEdit);
      end;
    end;
  end;
end;
...
begin
  ...
  FEdit.Left := cxGridInvisibleCoordinate; //don't show
  FEdit.Parent := FController.Site;        //here
...
procedure TcxGridEditingController.UpdateEditHeight(AHeight: Integer);
var
  R: TRect;
  AUpdateRow, ANeedUpdate: Boolean;
begin
  if CanUpdateEditValue and FEdit.ModifiedAfterEnter and not FEdit.IsHiding then
  begin
    R := FEdit.BoundsRect;
    R.Bottom := R.Top + AHeight;
    AUpdateRow := GridView.GetEditAutoHeight = eahRow;
    ANeedUpdate := (R.Bottom <> FEdit.BoundsRect.Bottom) and
      (AUpdateRow or (AHeight >= FInitialEditHeight));
    if ANeedUpdate and (R.Bottom < Controller.Site.ClientBounds.Bottom) then
    begin
      if AUpdateRow then
...
begin
  ...
  if FItem.Controller.IsClickedCell(Self) then
    Result := GridView.DoCellClick(Self, AButton, AShift)
  else
    Result := False;
  FItem.Controller.SetClickedCellInfo(nil);
  if Result then Exit;
...
var
  ARecord: TcxCustomGridRecord;
//  I, AFindLevel, ARecordIndex, AGroupCount: Integer;
begin
  ARecord := ViewData.Records[GetRecordIndex(AIndex)];
{ //!!!TODO:
  if AIndex = 0 then
  begin
    AGroupCount := ViewData.DataController.Groups.GroupingItemCount;
    if (AGroupCount > 0) and (ARecord.Level = AGroupCount) then
    begin
  ...
  AGroupCount := ViewData.DataController.Groups.GroupingItemCount;
  if (AGroupCount > 0) and (AIndex < AGroupCount) and
    (ARecord.Level > 0) and (ARecord.Level > AGroupCount - AIndex) then
  begin
    I := AGroupCount - AIndex;
    while I > 0 do
    begin
      ARecord := ViewData.Records[ARecord.ParentRecord.Index];
      Dec(I);
    end;
  end;
}
...
begin
  Result := GridViewInfo.GridView.Navigator.Visible;
end;
...
procedure TcxCustomGridTableItemOptions.SetEditAutoHeight(Value: TcxGridItemEditAutoHeight);
begin
...
begin
  Result := (FOptions.EditAutoHeight <> iaehNone) and
...
function TcxCustomGridTableItem.GetEditAutoHeight: TcxGridEditAutoHeight;
begin
...
begin
  GridView.Styles.GetViewParams(vsNavigator, nil, nil, Result);
end;
...
procedure TcxCustomGridTableOptionsBehavior.SetEditAutoHeight(Value: TcxGridEditAutoHeight);
begin
  if FEditAutoHeight <> Value then
  begin
    FEditAutoHeight := Value;
...
function TcxCustomGridTableView.GetEditAutoHeight: TcxGridEditAutoHeight;
begin
...
begin
  ...
  FNavigator := TcxGridViewNavigator.Create(Self);
end;
