...
{$ENDIF}
  ...
  cxVariants, cxCustomData, cxData, cxDataStorage, dxOffice11, 
...
type
  ...
  public
    ...
    property SmoothingMode: Integer read FSmoothingMode write FSmoothingMode;
  end;
...
function GetDefaultValueColor(AIndex: Integer): TColor; //todo: msn need port colors from .net
const
  ColorCount = 36;
...
procedure TcxGridChartSortBySeriesComboBox.DrawItem(AControl: TcxCustomComboBox;
  ...
  begin
    Result := ARect;
    AKeySize := ACanvas.TextHeight(dxMeasurePattern) - 2 * KeyOffset;
    with Result do
    begin
...
begin
  Result :=
    ...
    Canvas.TextHeight(dxMeasurePattern);
end;
...
begin
  ...
  FAlphaChannel := 255;
  FPenColor := clDefault; 
  FSmoothingMode := SmoothingModeAntiAlias;
end;
...
begin
  ...
  begin
    ...
    FCaptionHeight := Canvas.TextHeight(dxMeasurePattern);
  end;
...
var
  I, J: Integer;
  ASummaryValue, AMinSummaryValue, AMaxSummaryValue: Extended;
begin
  InitializeMinMaxValues(AMinValue, AMaxValue);
  AHasValuesOutOfRange := False;
  ...
  begin
    ...
    begin
      ...
      if (J = 0) or (ASummaryValue < AMinSummaryValue) then
        AMinSummaryValue := ASummaryValue;
    end;
    if Diagram.AxisValue.MinMaxValues = mmvCustom then
    begin
      AHasValuesOutOfRange := (AMinSummaryValue < AMinValue) or (AMaxSummaryValue > AMaxValue);
      ...
      begin
        AMinValue := AMinSummaryValue;
        AMaxValue := AMaxSummaryValue;
      end
      else
      begin
...
begin
  Result := Diagram.EmptyPointsDisplayMode = epdmGap;
  if Result then
    Result := GridView.ViewData.IsEmptyValue[ASeriesIndex, AValueIndex];
end;
...
begin
  if @FOnCustomDrawValueCaption <> @Value then
  begin
...
function TcxGridChartStackedColumnDiagram.HorizontalPaging: Boolean;
begin
  Result := False;
end;

function TcxGridChartStackedColumnDiagram.PagingInOppositeDirection: Boolean;
begin
  Result := not inherited PagingInOppositeDirection;
end;
