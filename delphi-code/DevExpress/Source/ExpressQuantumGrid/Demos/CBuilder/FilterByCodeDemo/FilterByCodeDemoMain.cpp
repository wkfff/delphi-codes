...
{
  SetFilter(
    (TUserFiltering)(((TcxComboBox*)Sender)->Properties->Items->Objects[((TcxComboBox*)Sender)->ItemIndex]));
}
//---------------------------------------------------------------------------
...
int TFilterByCodeDemoMainForm::GetFilterIndex(const TUserFiltering AFiltering)
{
  for (int I = 0; I < cbFilters->Properties->Items->Count; I++)
    if ((TUserFiltering)(cbFilters->Properties->Items->Objects[I]) == AFiltering)
      return I;
  return -1;
}
//---------------------------------------------------------------------------
