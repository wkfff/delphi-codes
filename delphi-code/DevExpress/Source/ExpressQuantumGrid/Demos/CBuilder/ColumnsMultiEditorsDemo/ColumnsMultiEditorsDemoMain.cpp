      ...
      TObject *Sender)
{
  ...
  try {
    clnSkill->DataBinding->ValueTypeClass = __classid(TcxStringValueType);
    clnGrade->DataBinding->ValueTypeClass = __classid(TcxVariantValueType);
    clnName->DataBinding->ValueTypeClass = __classid(TcxStringValueType);
  }  
  __finally {
    tvSkills->EndUpdate();
  }
