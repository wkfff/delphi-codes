...
type
  TfrmChartConnection = class(TfrmDemoBaisicMain)
...
begin
  cxPivotGridChartConnection.SourceData :=
    TcxPivotGridChartViewSourceData(TMenuItem(Sender).Tag);
end;

procedure TfrmChartConnection.miSourceColumnClick(Sender: TObject);
begin
  cxPivotGridChartConnection.SourceForCategories :=
    TcxPivotGridChartViewSourceForCategories(TMenuItem(Sender).Tag);
end;

procedure TfrmChartConnection.miChartColumDiagramClick(Sender: TObject);
begin
  cxGridChartView.ActiveDiagram := TcxGridChartDiagram(TMenuItem(Sender).Tag);
end;
