...
begin
  if (AIndex >= 0) and not FieldChooser.IsLocked then
  begin
    AField := FieldByIndex(AIndex);
      ...
      begin
        ...
        if ([odSelected, odFocused] * AState <> []) and (ItemIndex = AIndex) then
...
begin
  Style.Color := Painter.GetCustomizationFormListBackgroundColor;
end;
