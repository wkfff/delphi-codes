...
procedure TdxCustomNavBar.CheckCollapseState;
  ...
  begin
    Inc(FLockCheckBoundsCount);
    Width := AWidth;
    Dec(FLockCheckBoundsCount);
  end;
...
procedure TdxCustomNavBar.WMWindowPosChanged(var Message: TWMWindowPosChanged);
...
begin
  inherited;
  if IsSizeChanging and (FPainter <> nil) then
  begin
...
begin
  if IsSizeChanging and (FLockCheckBoundsCount = 0) and (FPainter <> nil) then
    with Message.WindowPos^ do
      Controller.CheckBounds(x, y, cx, cy);
  inherited;
end;
