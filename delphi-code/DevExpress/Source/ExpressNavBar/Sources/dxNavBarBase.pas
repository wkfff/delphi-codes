...
uses
  Windows, Classes, Controls, cxAccessibility, cxClasses, cxControls;
...
var
  ...
  APainter: TdxNavBarPainterAccess;
begin
// Requires
  Assert((Parent = nil) and (ADirection in [dirNone, dirDown]) or
    (Parent <> nil) and (ADirection <> dirNone) and not IsContainer);
//
  ANavBar := OwnerObjectControl as TdxCustomNavBar;
  APainter := TdxNavBarPainterAccess(ANavBar.Painter);
  APainter.RecreateViewInfo;
  APainter.RecalculateViewInfo; 
