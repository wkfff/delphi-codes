...
begin
  if not UseSkins or (SkinInfo.RibbonBackstageView = nil) then
...
begin
  if not UseSkins or (SkinInfo.RibbonBackstageViewMenu = nil) then
...
begin
  if not UseSkins or (SkinInfo.RibbonBackstageViewMenuButton = nil) then
...
begin
  if not UseSkins or (SkinInfo.RibbonBackstageViewTab = nil) then
...
begin
  if not UseSkins or (SkinInfo.RibbonBackstageViewTopLine = nil) then
...
procedure TdxSkinRibbonPainter.DrawButtonGroupSplitButtonSeparator(
  DC: HDC; const R: TRect; AState: Integer);
begin
  if not UseSkins or (SkinInfo.RibbonButtonGroupButton = nil) then
  ...
  begin
    ...
    try
      ...
      with SkinInfo.RibbonButtonGroupButton.ContentOffset.Rect do
        DrawButtonGroup(cxPaintCanvas.Handle, cxRectInflate(R, Left, 0, Right, 0), AState);
    finally
...
begin
  if not UseSkins or (SkinInfo.RibbonContextualTabSeparator = nil) then
...
begin
  if not UseSkins or (SkinInfo.RibbonContextualTabPanel = nil) then
...
begin
  if not UseSkins or (SkinInfo.RibbonFormCaption = nil) then
...
procedure TdxSkinRibbonPainter.DrawLaunchButtonDefaultGlyph(
  DC: HDC; const R: TRect; AState: Integer);
begin
  if not UseSkins then
...
begin
  if not UseSkins or (SkinInfo.RibbonMinimizeButtonGlyph = nil) then
...
procedure TdxSkinRibbonPainter.DrawTabGroupsArea(
  DC: HDC; const R: TRect; AIsQATAtBottom: Boolean);
begin
  if not UseSkins or (SkinInfo.RibbonTabPanel = nil) then
...
begin
  if not AUseAeroGlass and AHasApplicationButton and UseSkins then
  begin
...
begin
  ...
  begin
    if SkinManager.SkinByName(APainter.LookAndFeelName + dxRibbonStyleNamePrefixMap[rs2007]) = nil then
      SkinManager.AddSkin(TdxSkinRibbonPainter.Create(TdxSkinLookAndFeelPainter(APainter)));
    if SkinManager.SkinByName(APainter.LookAndFeelName + dxRibbonStyleNamePrefixMap[rs2010]) = nil then
      SkinManager.AddSkin(TdxSkinRibbon2010Painter.Create(TdxSkinLookAndFeelPainter(APainter)));
  end;
end;
