...
type
  ...
  public
    ...
    class function PanelContentExtents(APanel: TdxStatusBarPanel; AHasBorders: Boolean): TRect; override;
...
begin
  if not AHasBorders then
    ...
    if GetSkinInfo(APanel.StatusBarControl, AInfo) and (AInfo.LinkBorderPainter <> nil) then
      Result := AInfo.LinkBorderPainter.ContentOffset.Rect
    else
      Result := inherited PanelContentExtents(APanel, AHasBorders);
end;
