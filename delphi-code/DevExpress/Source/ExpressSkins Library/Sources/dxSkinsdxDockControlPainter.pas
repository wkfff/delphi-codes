...
type
  ...
  protected
    ...
    function GetSkinLookAndFeelPainter: TcxCustomLookAndFeelPainter;
  ...
  public
    ...
    function GetCaptionButtonsIndents: TRect; override;
...
function TdxDockControlSkinPainter.GetCaptionButtonsIndents: TRect;
begin
  Result := Rect(0, 2, 0, 2);
end;
...
begin
  ...
  begin
    ...
    Result := AElement.Size.cy - (ABorders.Top + ABorders.Bottom);
    Inc(Result, Integer(Odd(Result + GetCaptionButtonSize)));
  end;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      AElement.Draw(ABitmap.Canvas.Handle, ABitmap.ClientRect, Integer(IsWorkingControl(AControl)));
      RotateBitmap(ABitmap, False);
  ...
  begin
    ...
    try
      ...
      if APosition = tctpBottom then
        ABitmap.Rotate(ra0, True);
      AElement.Draw(ABitmap.Canvas.Handle, ABitmap.ClientRect, 0, AStates[IsActive]);
      if APosition = tctpBottom then
        ABitmap.Rotate(ra0, True);
      cxBitBlt(ACanvas.Handle, ABitmap.Canvas.Handle, ARect, cxNullPoint, SRCCOPY);
    finally
...
function TdxDockControlSkinPainter.GetSkinLookAndFeelPainter: TcxCustomLookAndFeelPainter;
var
  I: Integer;
begin
  Result := nil;
...
begin
  APainter := GetSkinLookAndFeelPainter;
  Result := (APainter <> nil) and APainter.GetPainterData(AData);
end;
