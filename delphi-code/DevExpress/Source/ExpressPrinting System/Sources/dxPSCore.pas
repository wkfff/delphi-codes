...
uses
  ...
  ActiveX, Menus, Forms, ImgList, Contnrs,
  dxMessages, dxCore, cxClasses, cxControls,
  cxGraphics, cxButtons, cxGeometry, dxBase, dxPSGlbl, dxPSSngltn, dxPSEngn,
  dxPSESys, dxPSForm, dxBkgnd, dxPrnPg, dxPgsDlg, dxPrnDlg, dxWrap, IniFiles,
  cxDrawTextUtils, dxPSFillPatterns, dxPSReportRenderCanvas, dxPSPDFExportCore;
...
type
  ...
  public
    ...
    function CalculateFootnotesHeight: Integer; virtual;
...
{$ENDIF}
  ...
  protected
    ...
    procedure PrepareFont(AFont: TFont; AAdjustOnScale: Boolean);
  ...
  public
    ...
    procedure FillRoundRectEx(ACanvas: TdxPSReportRenderCustomCanvas;
      ...
      AForeColor, ABackColor: TColor; APattern: TdxPSFillPatternClass); virtual;    
    procedure FillRgn(ACanvas: TdxPSReportRenderCustomCanvas; Rgn: HRGN; AColor: TColor); virtual;
    procedure FillRgnEx(ACanvas: TdxPSReportRenderCustomCanvas; Rgn: HRGN;
      AForeColor, ABackColor: TColor; APattern: TdxPSFillPatternClass); virtual;
  ...
  protected
    ...
    procedure DoReportLinkDestroyed(ALink: TBasedxReportLink); virtual;
  ...
  protected
    ...
    procedure DoReportLinkDestroyed(ALink: TBasedxReportLink); override;
  ...
  public
    ...
    function FindDataItemByData(AData: Integer; var ADataItem: TAbstractdxReportCellData): Boolean;
  ...
  protected
    ...
    function GetRealReportLinkAndPageIndex(var APageIndex: Integer; var ALink: TBasedxReportLink): Boolean;
  public
...
type
  ...
  protected
    procedure DoReportLinkDestroyed(ALink: TBasedxReportLink);
...
procedure TdxPSReportLinkList.DoReportLinkDestroyed(ALink: TBasedxReportLink);
var
  I: Integer;
begin
  for I := 0 to Count - 1 do
    Items[I].DoReportLinkDestroyed(ALink);
end;

procedure TdxPSReportLinkList.Notify(Ptr: Pointer; Action: TListNotification);
begin
  inherited Notify(Ptr, Action);
  if Action = lnDeleted then
    DoReportLinkDestroyed(TBasedxReportLink(Ptr));
end;
...
begin
  Result.Top := ConvertValue(AValue.Top);
  Result.Left := ConvertValue(AValue.Left);
  Result.Right := ConvertValue(AValue.Right);
  Result.Bottom := ConvertValue(AValue.Bottom);
end;

function ConvertPoint(const AValue: TPoint): TPoint;
begin
  Result.X := ConvertValue(AValue.X);
  Result.Y := ConvertValue(AValue.Y);
end;
...
begin
  ...
  Inc(ContentBounds.Right, RowHeaderBounds.Right - RowHeaderBounds.Left);
...
begin
  ...
  TitleOffset := Point(R.Left, R.Bottom);
end;
...
procedure TdxPSPageRenderInfo.CalculateOffsets;
  ...
  begin
    ...
    begin
      ...
      if HasTitle then
        Dec(APaintSize, cxRectHeight(RenderInfo.TitleBounds));
      if HasFootnotes then
        Dec(APaintSize, cxRectHeight(RenderInfo.FootnotesBounds));
...
begin
  ...
  AFullRect := ScaleRect(AFullRect, RenderInfo.ScaleFactor, 100, RenderInfo.ScaleFactor, 100);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if ScaleValue(ADelimiter - AOffset) > AContentSize then
          Break;
...
begin
  Result := ReportRenderInfo.CalculatePageContentHeight(APageIndex);
end;
...
begin
  ...
  begin
    if PageRenderInfos[I].IsEmptyPage then
      Inc(Result);
  end;
end;
...
begin
  FootnotesBounds.Right := MulDiv(PaintSize.X, 100, ScaleFactor);
  FootnotesBounds.Bottom := MulDiv(CalculateFootnotesHeight,
...
procedure TdxPSReportRenderInfo.CalculatePageRealAndVirtualIndexes(
  APageIndex: Integer; out AVirtualPageIndex, ARealPageIndex: Integer);
begin
...
begin
  Result := CalculateReportNotesTextHeight(FootnotesText, FootnotesFont, FootnotesAdjustOnReportScale);
...
begin
  Result := CalculateReportNotesTextHeight(TitleText, TitleFont, TitleAdjustOnReportScale);
...
begin
  Result := CalcTextHeight(ACanvas, 'Wg', False, AFont, -1);
end;
...
begin
  Result := CalcTextRect(ACanvas, AText, ARect, CalcFormats[AWordBreak], AFont)
end;
...
procedure TdxPSReportRenderer.DrawTextEx(ACanvas: TdxPSReportRenderCustomCanvas;
  ...
  begin
    Result := R;
    if not EqualRect(AIndents, cxNullRect) then
    begin
      Dec(Result.Bottom, ConvertValue(AIndents.Bottom));
      Dec(Result.Right, ConvertValue(AIndents.Right));
      Inc(Result.Left, ConvertValue(AIndents.Left));
      Inc(Result.Top, ConvertValue(AIndents.Top));
    end;
  end;
...
procedure TdxPSReportRenderer.FillRgn(
  ACanvas: TdxPSReportRenderCustomCanvas; Rgn: HRGN; AColor: TColor);
begin
...
procedure TdxPSReportRenderer.FillRgnEx(ACanvas: TdxPSReportRenderCustomCanvas;
  Rgn: HRGN; AForeColor, ABackColor: TColor; APattern: TdxPSFillPatternClass);
begin
...
begin
  ...
  try
    RenderPageEx(ARenderCanvas, APageBounds,
      APageIndex, AContinuousPageIndex, AZoomFactor);
  finally
    ARenderCanvas.Free;
  end;
end;
...
begin
  ...
  if RenderInfo.CanRenderPage(VirtualPageIndex) then
  begin
    ...
    try
      ...
      RenderPageReportArea(RealPageIndex);
    finally
      FRenderStage := [];
      UnprepareRenderPage;
    end; 
...
var
  R: TRect;
  APoint: TPoint;
  ASaveOrg, APrevWindowOrg: TPoint;
  I, H, StartIndex, EndIndex: Integer;
begin
  APoint := PageRenderInfo.DataOffset;
  Inc(APoint.X, LineThickness);
  Inc(APoint.Y, LineThickness);
  APrevWindowOrg := Canvas.OffsetWindowOrg(cxPointInvert(APoint));
  try
    ...
    Canvas.SaveState;
    try
      Canvas.IntersectClipRgn(R);
      RenderPageContentPart(ReportCells.HeaderCells, 0, -1,
        cxRectOffset(PageRenderInfo.HeaderBounds, PageRenderInfo.DetailBounds.Left, 0));
      RenderPageContentPart(ReportCells.RowHeaderCells, 0, -1,
        cxRectOffset(PageRenderInfo.RowHeaderBounds, 0, PageRenderInfo.DetailBounds.Top));
      RenderPageContentPart(ReportCells.HeaderCornerCells, 0, -1,
        PageRenderInfo.HeaderCornerBounds);
      if PageRenderInfo.HasHeader then
        Canvas.ExcludeClipRect(PageRenderInfo.HeaderBounds);
      ASaveOrg := Canvas.WindowOrg;
      Canvas.SaveClipRgn;
      if PageRenderInfo.HasRowHeader then
        Canvas.ExcludeClipRect(cxRectOffset(PageRenderInfo.RowHeaderBounds, 0, PageRenderInfo.DetailBounds.Top));
      if PageRenderInfo.HasHeaderCorner then
        Canvas.ExcludeClipRect(PageRenderInfo.HeaderCornerBounds);
      Canvas.OffsetWindowOrg(cxPoint(-cxRectWidth(PageRenderInfo.RowHeaderBounds),
      ...
      begin
        ...
        begin
          StartIndex := PageRenderInfo.IndexPairs[I].StartIndex;
          EndIndex := PageRenderInfo.IndexPairs[I].EndIndex;
          RenderPageContentPart(PageRenderInfo.ReportCells.Cells, StartIndex, EndIndex, PageRenderInfo.DetailBounds);
        end;
        ...
        if not PageRenderInfo.IsBottomPage then
          H := PageRenderInfo.DetailBounds.Bottom
        else
          if PageRenderInfo.ReportCells.Cells.CellCount > EndIndex then
            H := ReportCells.Cells[EndIndex].BoundsRect.Bottom;
        Dec(H, PageRenderInfo.DetailBounds.Top);
        Canvas.OffsetWindowOrg(Point(0, -H));
      end;
      Canvas.WindowOrg := ASaveOrg;
      Canvas.RestoreClipRgn;
      Canvas.OffsetWindowOrg(Point(0, -cxRectHeight(PageRenderInfo.HeaderBounds)-H));
...
begin
  ...
  begin
    ...
    FCreator := TdxReportItem(Source).Creator;
  end
  else
...
procedure TdxReportItem.DoReportLinkDestroyed(ALink: TBasedxReportLink);
begin
  if ALink = FCreator then
    FCreator := nil;
end;
...
begin
  ...
  begin
    while DataItemCount > 0 do
      DataItems[DataItemCount - 1].Free;
    DataListRelease;
  end;
end;
...
function TdxReportCell.FindDataItemByData(
  AData: Integer; var ADataItem: TAbstractdxReportCellData): Boolean;
var
  I: Integer;
begin
  Result := False;
...
procedure TdxReportCell.DoReportLinkDestroyed(ALink: TBasedxReportLink);
var
  I: Integer;
begin
  inherited DoReportLinkDestroyed(ALink);
  for I := 0 to CellCount - 1 do
    Cells[I].DoReportLinkDestroyed(ALink);
  for I := 0 to DataItemCount - 1 do
    DataItems[I].DoReportLinkDestroyed(ALink);
end;
...
begin
  if ShowCaption and (LookAndFeel <> nil) then
    Inc(R.Top, LookAndFeel.GetCaptionHeight(Self) div 2);
end;
...
begin
  ...
  if AReader.PSVersion.Minor >= 48 then
  begin
    if AReader.ReadBoolean then
...
begin
  ...
  if AWriter.PSVersion.Minor >= 48 then
  begin
...
procedure TdxReportCells.DoReportLinkDestroyed(ALink: TBasedxReportLink);
begin
  Cells.DoReportLinkDestroyed(ALink);
  HeaderCells.DoReportLinkDestroyed(ALink);
  FooterCells.DoReportLinkDestroyed(ALink);
  RowHeaderCells.DoReportLinkDestroyed(ALink);
  HeaderCornerCells.DoReportLinkDestroyed(ALink);
end;
...
begin
  Result := CXTO_AUTOINDENTS or CXTO_EXPANDTABS or
    CXTO_PATTERNEDTEXT or CXTO_CHARBREAK or CXTO_EDITCONTROL;
end;
...
procedure TdxReportCellText.DrawText(ACanvas: TdxPSReportRenderCustomCanvas);
  ...
  function CalculateTextWidth(AFont: TFont; const S: string): Integer;
  begin
    ACanvas.Font.Assign(AFont);
    Result := ACanvas.TextSize(S).cx;
  end;

  procedure CalculateOptimalFont(AFont: TFont; const R: TRect; const S: string);
  var
    ATextWidth: Integer;
    L, H: Integer;
  begin
    if CalculateTextWidth(AFont, S) > cxRectWidth(R) then
    begin
      H := AFont.Size;
      L := MulDiv(H, 2, 3);
      while L <= H do
      begin
        ...
        ATextWidth := CalculateTextWidth(AFont, S);
      ...
      try
        ...
        begin
          ...
          try
            ...
            CalculateOptimalFont(ATempFont, R, S);
...
function TdxReportCellText.MeasureContentHeight(
  ACanvas: TdxPSReportRenderCustomCanvas): Integer;
var
  R: TRect;
begin
...
procedure TdxReportCellText.ReadData(AReader: TdxPSDataReader);
  ...
  function HasIndentsProperty(const AVersion: TdxPSVersion): Boolean;
  begin
    Result := (AVersion.Major >= 4) and (AVersion.Minor >= 48);
  end;
...
begin
  ...
  if HasIndentsProperty(AReader.PSVersion) then
...
begin
  ...
  begin
    ...
    begin             
      Result.Combine(TcxRegion.Create(R1), roAdd);
      if AIsVertical then
...
begin
  ...
  try
    ...
    begin
      ARegion.Combine(
        TcxRegion.Create(cxRectInflate(ButtonRect, LineThickness, LineThickness)),
        roSubtract);
    end;
    ARegion.Combine(TcxRegion.Create(CellButton.BoundsRect), roIntersect);
...
procedure TBasedxReportLink.DoReportLinkDestroyed(ALink: TBasedxReportLink);
begin
  if Assigned(ReportCells) then
    ReportCells.DoReportLinkDestroyed(ALink);
end;
...
begin
  ...
  if Assigned(ComponentPrinter) then
  begin
    AComposition := ComponentPrinter.CurrentCompositionByLink(Self);
    if Assigned(AComposition) then
...
begin
  if Assigned(FOnDataSourceChanged) then
    FOnDataSourceChanged(Self);
...
function TdxPSCompositionReportRenderInfo.RealPageIndexToVirtualPageIndex(
  APageIndex: Integer; ATakeIntoAccountEmptyPages: Boolean): Integer;
var
  Item: TdxCompositionLinkItem;
begin 
  ...
  if Item = nil then
    ...
    Result := Item.ReportLink.RealPageIndexToVirtualPageIndex(
      APageIndex, ATakeIntoAccountEmptyPages);
end;
...
function TdxPSCompositionReportRenderInfo.GetRealReportLinkAndPageIndex(
  var APageIndex: Integer; var ALink: TBasedxReportLink): Boolean;
var
  AItem: TdxCompositionLinkItem;
begin
  GetCompositionLinkItemByPageIndexAndFixIndex(APageIndex, APageIndex, AItem);
  Result := Assigned(AItem) and Assigned(AItem.ReportLink);
  if Result then
    ALink := AItem.ReportLink;
end;
...
var
  ARealReportLink: TBasedxReportLink;
begin
  if RenderInfo.GetRealReportLinkAndPageIndex(APageIndex, ARealReportLink) then
  begin
    if not ReportLink.ContinuousPageIndexes then
      dxHFFormatObject.TotalPages := ARealReportLink.PageCount;
    ARealReportLink.PaintPageEx(ACanvas, APageBounds,
      APageIndex, AContinuousPageIndex, AZoomFactor);
  end;
end;
...
const
  ...
  procedure PlaceButton(X, AWidth: Integer; AButton: TcxButton);
  begin
    AButton.SetBounds(X, ClientHeight -
      MulDiv(btnOffsetY, Screen.PixelsPerInch, 96) -
      MulDiv(btnHeight, Screen.PixelsPerInch, 96), AWidth,
      MulDiv(btnHeight, Screen.PixelsPerInch, 96));
  end;

var
  AWidth, ALargeWidth, AOffsetX, AFirstOffsetX: Integer;
begin
  AFirstOffsetX := MulDiv(btnFirstOffsetX, Screen.PixelsPerInch, 96);
  ...
  begin
    PlaceButton(ClientWidth - AFirstOffsetX - AWidth, AWidth, btnHelp);
    PlaceButton(btnHelp.Left - AOffsetX - AWidth, AWidth, btnApply);
  end    
  else
    PlaceButton(ClientWidth - AFirstOffsetX - AWidth, AWidth, btnApply);

  PlaceButton(btnApply.Left - AOffsetX - AWidth, AWidth, btnCancel);
  PlaceButton(btnCancel.Left - AOffsetX - AWidth, AWidth, btnOK);
  PlaceButton(AFirstOffsetX, ALargeWidth, btnRestoreOriginal);
  PlaceButton(btnRestoreOriginal.BoundsRect.Right + AOffsetX, ALargeWidth, btnRestoreDefaults);
  PlaceButton(AFirstOffsetX, ALargeWidth, btnTitleProperties);
  PlaceButton(btnTitleProperties.BoundsRect.Right + AOffsetX, ALargeWidth, btnFootnoteProperties);
end;
...
procedure TdxPSCustomPreviewWindow.Release;
begin
  Form.Free;
end;
...
function TdxPSCustomPreviewWindow.GetForm: TForm;
begin
...
const
  PageRangeMap: array[Boolean] of TdxPageRanges = (prAll, prRange);
begin
  if PageIndexesAsString <> AValue then
  begin
    DecodePageIndexes(AValue, FPageIndexes);
    PageRanges := PageRangeMap[Length(FPageIndexes) > 0];
  end;
end;
...
begin
  if cpsPrinting in State then
    AbortPrinting := True;
  if Assigned(FPreviewWindow) then
    cxReleaseForm(FPreviewWindow);
end;
...
procedure TCustomdxComponentPrinter.SetupPreviewProperties(APreviewWindow: TdxPSCustomPreviewWindow);
  ...
  procedure PreparePreviewWindowForm(AForm: TForm);
  begin
  {$IFDEF DELPHI9}
...
begin
  if not IsDesigning then
  begin
    PreparePreviewWindowForm(APreviewWindow.Form);
