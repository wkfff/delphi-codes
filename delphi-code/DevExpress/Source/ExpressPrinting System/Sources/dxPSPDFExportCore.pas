...
procedure TdxPSPDFPageContent.AdjustFontSize(
  const AText: WideString; const ATextWidth: Double);
var
  ATempWidth: Integer;
begin
...
procedure TdxPSPDFPageContent.TextOut(const AText: WideString;
  const X, Y, ACharsSpacing, AWordSpacing: Double);
  ...
  const
    D2P = 96 / 72;
  begin
    Result := ACharsSpacing * (Length(AText) - 1) + TextWidth(AText) / D2P;
  end;

  function ExtractWord(AStart, AEnd: PWideChar): WideString;
  begin
    SetString(Result, AStart, (DWORD(AEnd) - DWORD(AStart)) div SizeOf(WideChar));
  end;

  function PlaceText(X, Y: Double): Double;
  var
    ACursor, AEndScan, ANextWordBreak: PWideChar;
    AWordWidth: Double;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        AWordText := ExtractWord(ACursor, ANextWordBreak);
        ACursor := ANextWordBreak;

        AWordWidth := CalculateTextWidth(AWordText);
        if (Length(AWordText) = 1) and cxGetIsWordDelimeter(Font.CodePage, AWordText[1]) then
          AWordWidth := AWordWidth + ACharsSpacing
        else
          WriteText(AWordText);
...
begin
  ...
  try
    ...
    ATextWidth := PlaceText(X, PageHeight - Y - FontSize);
...
procedure TdxPSPDFPageContent.Pie(const R: TRect;
  const APoint1, APoint2: TPoint; AColor: TColor);
begin
