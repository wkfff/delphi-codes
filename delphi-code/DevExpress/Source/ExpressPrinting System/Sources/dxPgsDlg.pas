...
begin
  ...
  FHFToolBarHelper.CenteringToolbar := True;
...
begin
  Result := Page.PageHeader.TextAlignY[SelectedTitlePart];
end;

procedure TdxfmPageSetupDialog.SetSelectedTitlePartTextAlignY(AAlign: TcxTextAlignY);
begin
  Page.PageHeader.TextAlignY[SelectedTitlePart] := AAlign;
end;
...
procedure TdxfmPageSetupDialog.SetupDialog(const APageSetupDlgData: TdxPageSetupDlgData);
  ...
  begin
    ...
    begin
      pnlStyleName.Enabled := not FSavePrintStyle.BuiltIn;
...
begin
  if ActiveControl is TcxCustomMemo then
    Result := GetTitlePart(ActiveControl)
  else
    if ActiveControl is TcxCustomInnerMemo then
      Result := GetTitlePart(GetInnerControlContainer(ActiveControl))
    else // todo: for tests
