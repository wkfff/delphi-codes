...
uses
   {$IFDEF DELPHI6} 
    DesignIntf, {$IFDEF CBUILDER10}DesignEditors,{$ENDIF}
   {$ELSE}
    DsgnIntf,
   {$ENDIF}
   {$IFDEF DELPHI5}
    {$IFNDEF CBUILDER10}DBReg,{$ENDIF}
   {$ENDIF}
  Classes, DB, dxPSGlbl, dxPSDBBasedXplorer;
...
procedure Register;
begin
  RegisterComponents(sdxPSComponentPage, [TdxPSDBBasedExplorer]);
