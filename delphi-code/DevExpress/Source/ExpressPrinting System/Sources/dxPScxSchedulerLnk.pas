...
uses
  ...
  ImgList, Menus, cxGraphics, cxClasses, cxControls, cxStyles, cxLookAndFeels,
  cxLookAndFeelPainters, cxScheduler, cxEdit, cxContainer, cxTextEdit, cxMaskEdit,
  cxDropDownEdit, cxCalendar, cxCheckBox, cxImageComboBox, cxTimeEdit, cxSpinEdit,
  cxDateUtils, dxPSCore, dxPSGlbl, dxExtCtrls, dxPScxCommon, dxPrevw, dxPgsdlg,
  dxPrnPg, cxSchedulerUtils, cxSchedulerStorage, cxSchedulerCustomControls,
  cxSchedulerCustomResourceView, cxSchedulerDayView, cxSchedulerDateNavigator,
  cxSchedulerWeekView, cxSchedulerTimeGridView, cxSchedulerYearView, cxGroupBox,
  cxGeometry, cxDrawTextUtils, cxLabel, cxButtons, cxPC, cxSchedulerGanttView, 
...
uses
  Messages, ShlObj, dxPSUtl, dxPSRes, dxPSPopupMan, Math, dxCore,
{$IFDEF DELPHI6}
  DateUtils, Types,
{$ENDIF}
  ActiveX, CommCtrl;
...
begin
  ...
  if ActiveStyle <> nil then
    Result := GetDateTimeIF(ActiveStyle.CheckPrintRangeDate(
      DateOf(FDatePrint[AIndex]), AIndex), DefaultDate)
  else
...
begin
  ...
  if ActiveStyle <> nil then 
    Result := TimeOf(GetDateTimeIF(ActiveStyle.CheckPrintRangeTime(
...
begin
  ...
  try
    Result := ReportLink.Renderer.CalcTextHeight(ARenderCanvas, 'Wg',
...
begin
  CreateAdditionalControls(ASite, True, ShowTaskPad,
    ShowNotesAreaBlank, ShowNotesAreaLined, False, ABounds);
  InitReportCell(CreateSchedulingAreaCell(ASite), ABounds);
end;
  ...
  case FCurrentSection of
    ...
    begin
      if HasDailySection then
        ADate := StartOfTheWeek(APageIndex + PrintRange.PrintFrom)
      else
...
function TcxSchedulerReportLinkWeeklyPrintStyle.GetDateForPage(
  APageIndex: Integer): TDateTime;
begin
  Result := StartOfTheWeek(PrintRange.PrintFrom  + (APageIndex div GetResourcePageCount div (Byte(Layout) + 1)) * 7);
end;
...
begin
  ACanvas := TdxPSReportRenderScreenCanvas.Create;
  try
    FLineHeight := Renderer.CalcTextHeight(ACanvas, 'Wg', False, Font);
...
var
  AStartGroup, AFinishGroup, ATotalCount, AResPerPage: Integer;
begin
  AStartGroup := 0;
  ...
  begin
    ...
    if (AStartGroup = AFinishGroup) and (ViewInfo.GroupLineCount[AStartGroup] > ATotalCount) then
    begin
      ATotalCount := 0;
      while ATotalCount < ViewInfo.GroupLineCount[AStartGroup] do
      begin
...
begin
  Result := Max(1, cxRectHeight(GetPageContentBounds(ARow, 0)) div
...
begin
  ...
  Result := TcxSchedulerReportEventCell.CreateFrom(AHost, AEventViewInfo);
end;
