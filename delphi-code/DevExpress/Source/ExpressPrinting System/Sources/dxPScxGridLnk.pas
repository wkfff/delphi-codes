...
uses
  ...
  cxCheckBox, cxImage, dxBase, dxPSSngltn, dxExtCtrls, dxPSCore, dxPrnPg,
  dxPScxCommon, dxCore, cxContainer, cxLabel, cxPC, cxMaskEdit, cxDropDownEdit,
  cxSpinEdit, cxColorComboBox, cxButtons, dxPSReportRenderCanvas, cxGeometry;
...
type
  ...
  protected
    ...
    procedure AddSummaryGroupInfo(AItem: TcxDataSummaryItem; const ADisplayText: string);
    procedure AddSubItems(AParent: TdxReportCell); override;
...
begin
  ...
  begin
    ...
    if Assigned(ARecord) and (ARecord.Index = 0) then
...
function TdxGridCustomLayoutViewFormatter.GetRecordClass(AGridRecord: TcxGridCustomLayoutRecord): TdxReportCustomLayoutRecordClass;
begin
  Result := TdxReportCustomLayoutRecord;
end;

procedure TdxGridCustomLayoutViewFormatter.DoInitializeRecord(ARecord: TdxReportCustomLayoutRecord; AGridRecord: TcxGridCustomLayoutRecord);
begin
  ARecord.Data := Integer(AGridRecord);
end;
...
procedure TdxGridCardViewFormatter.DoInitializeRecord(ARecord: TdxReportCustomLayoutRecord; AGridRecord: TcxGridCustomLayoutRecord);
begin
  inherited;
  with ReportLink.OptionsCards.Shadow do
  begin
    ARecord.ShadowColor := ActualColor;
    ARecord.ShadowDepth := Depth;
    (ARecord as TdxReportCard).ShadowPosition := Position;
  end;

  SetViewParams(ARecord, GetCardViewParams(AGridRecord));

  ARecord.Width := Adapter.CardWidth;
  (ARecord as TdxReportCard).CreateLayers(Builder);
end;
...
constructor TdxReportCustomLayoutRecord.CreateEx(AParent: TdxReportCell; AGridRecord: TcxGridCustomLayoutRecord);
begin
  Create(AParent);
  Data := Integer(AGridRecord);
end;
...
begin
  for I := GetSubItemCount - 1 downto 0 do
    ...
    begin
      Item := CreateSubItem(I, AParent);
      Item.BoundsRect := SubItemBounds[I]; // needed before item initialization
      Item.Data := Integer(SummaryItemInfos[I]);
      InitializeSubItem(Item, I);
    end;
end;

procedure TdxGridTableViewGroupRowProducer.AddSummaryGroupInfo(
  AItem: TcxDataSummaryItem; const ADisplayText: string);
var
  AInfo: TcxGridSummaryItemInfo;
begin                                                                 
  if ADisplayText = '' then Exit;
...
begin
  ...
  dxPSUtl.dxCopyList(DelimitersHardHorz, AHorzDelimiters);
...
begin
  if (ReportRowCount = 0) or not ReportRows[0].FindDataItemByData(Integer(AColumn), Result) then
    Result := nil;
end;
