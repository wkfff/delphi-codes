...
procedure TdxPSPDFMetaFileExportProvider.EmfExtTextOut(
  ...
  procedure CalculateTextParams(out ATextRect: TRect; out ATextWidth: Integer);
  begin
    ATextRect := ARecord^.rclBounds;
    if (ATextRect.Right < 0) and (ATextRect.Bottom < 0) then
    begin
      ATextRect := Bounds(0, 0, CalculateTextWidth(ARecord, ARecord^.emrtext), Abs(PDFCanvas.Font.Size));
      ATextRect := cxRectOffset(ATextRect, ARecord^.emrtext.ptlReference);
    end;
    Inc(ATextRect.Bottom, Abs(PDFCanvas.Font.Size));
    ATextRect := ConvertRect(ATextRect);
    if ARecord^.emrtext.fOptions and ETO_CLIPPED = 0 then
      ATextWidth := cxRectWidth(ATextRect)
    else
      ATextWidth := CalculateTextWidth(ARecord, ARecord^.emrtext);
  end;
  ...
  begin
    ...
    begin
      ...
      Result := dxAnsiStringToWideString(ATextA, dxGetCodePageFromCharset(PDFCanvas.Font.Charset));
    end;
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      CalculateTextParams(R, ATextWidth);
      PDFCanvas.DrawTextLine(R, R, ATextWidth,
...
begin
  ...
  try
    ...
    begin
      ...
      PageContent.Polyline(P, PenWidth, Pen.Color);
...
function TdxPSPDFMetaFileExportProvider.ExtractBitmap(ARecordAddress: Pointer;
...
var
  ABitmap: TcxBitmap;
begin
  ABitmap := ExtractBitmap(ARecordAddress, ASource, ABitmapInfoOffset, ABitmapBitsOffset, AColorUsage);
  try
