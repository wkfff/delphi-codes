...
uses
  ...
  StdCtrls, Buttons, ExtCtrls, ImgList, cxControls, cxGraphics, cxGeometry,
  cxLookAndFeels, cxLookAndFeelPainters, dxGDIPlusAPI, dxGDIPlusClasses;
...
type
  ...
  TdxCustomOrgChart = class(TcxControl, IdxSkinSupport)
  private
...
procedure TdxCustomOrgChart.DefaultDrawNode(
  Node: TdxOcNode; ACanvas: TCanvas; R: TRect);
var
  ABitmap: TcxBitmap;
  ABitmapRect: TRect;
  P: TPoint;
begin
  ...
  begin
    if IsZoomed then
    begin
      ABitmap := TcxBitmap.CreateSize(Images.Width, Images.Height);
      try
        ABitmapRect := cxRectBounds(P.X, P.Y, DoZoom(ABitmap.Width), DoZoom(ABitmap.Height));
        cxStretchBlt(ABitmap.Canvas.Handle, ACanvas.Handle, ABitmap.ClientRect, ABitmapRect, SRCCOPY);
        Images.Draw(ABitmap.Canvas, 0, 0, Node.ImageIndex);
        cxStretchBlt(ACanvas.Handle, ABitmap.Canvas.Handle, ABitmapRect, ABitmap.ClientRect, SRCCOPY);
      finally
        ABitmap.Free;
      end;
    end
...
{$ENDIF}
  if FDrag and (ocCanDrag in Options) and not Dragging and ((AShift = [ssLeft]) or (AShift = [ssLeft, ssCtrl]))
...
var
  ...
  R: TRect;
begin
  R := Rect(Min(X1, X2), Min(Y1, Y2), Max(X1, X2) + 1, Max(Y1, Y2) + 1);
  AGraphics := dxGpBeginPaint(ACanvas.Handle, R);
  try
    AGraphics.SmoothingMode := SmoothingModeAntiAlias;
