...
type
  ...
  protected
    ...
    function IsTabBorderThick(ATabVisibleIndex: Integer): Boolean; override;
    function UseLookAndFeelCloseButton: Boolean; override;
  public
...
function TcxPCTabsPainter.GetTabCorrection(ATabVisibleIndex: Integer): TcxPCRectCorrection;
const
...
begin
  if not TabViewInfo[ATabVisibleIndex].IsMainTab then
    Result := roIntersect
  else
    Result := roSet;
end;
...
begin
  ...
  ATabViewInfo := TabViewInfo[ATabVisibleIndex];
  if IsNativePainting and ATabViewInfo.HasCloseButton then
  begin
...
begin
  ...
  begin
    if ViewInfo.IsTabsContainer then
      ...
      if (ALineIndexBoundsA[ATabViewInfo.VisibleRow].Right = ATabViewInfo.VisibleIndex) then
    ...
    if ViewInfo.IsTabsContainer then
      ...
      if ALineIndexBoundsA[ATabViewInfo.VisibleRow].Left = ATabViewInfo.VisibleIndex then
        ...
        if ATabViewInfo.VisibleIndex = ViewInfo.TabsViewInfo.ViewInfoCount - 1 then
          ATabPositionWithinRow := tprRightMost
        else
          ATabPositionWithinRow := tprMiddle;

  // to work like standard PageControl
  if not ViewInfo.MultiLine or ViewInfo.RaggedRight then
    if ATabViewInfo.IsMainTab and (ATabPositionWithinRow = tprRightMost) then
      ATabPositionWithinRow := tprMiddle;

  PartId := ATabNativePartA[ATabViewInfo.IsMainTab, ATabPositionWithinRow];
...
var
  ...
  begin
    ...
    ExtendRect(ATabsPosition.ExtendedTabsRect, RotateRect(Rect(0, ATabsContainerOffset.Top, 0, 0), ViewInfo.TabPosition));
    with ViewInfo do
    begin
...
begin
  if IsNativePainting and CanDrawParentBackground and ViewInfo.ParentBackground then
    cxDrawTransparentControlBackground(ViewInfo.IControl.GetControl, ABitmap.cxCanvas, ARect)
  else
...
function TcxPCTabsPainter.UseLookAndFeelCloseButton: Boolean;
begin
  Result := True;
end;
  ...
  with FButtonsRect do
    ...
    begin
      if ViewInfo.NavigatorPosition in [npLeftTop, npLeftBottom] then
        ...
        Left := ViewInfo.Width - AButtonsWidth - GetButtonsRegionWOffset;
      if ViewInfo.TabPosition = tpTop then
        Top := GetButtonsRegionHOffset
      else
        Top := ViewInfo.Height - AButtonHeight - GetButtonsRegionHOffset;
    ...
    begin
      if ViewInfo.TabPosition = tpLeft then
        ...
        Left := ViewInfo.Width - AButtonHeight - GetButtonsRegionHOffset;
      if ViewInfo.NavigatorPosition in [npLeftTop, npRightTop] then
        Top := GetButtonsRegionWOffset
      else
        Top := ViewInfo.Height - AButtonsWidth - GetButtonsRegionWOffset;
...
begin
  ...
  begin
    ViewInfo.NavigatorButtons := NavigatorButtons;
    Exit;
  end;
  ...
  begin
    AButton := ViewInfo.NavigatorButtonInfos[I].ButtonType;
    AButtonBounds := ViewInfo.NavigatorButtonInfos[I].Bounds;
    AButtonBounds.TopLeft := AButtonLeftTopCorner;
    if AIsButtonsRotated then
    begin
      AButtonBounds.Bottom := AButtonBounds.Top + GetButtonWidth(AButton);
      AButtonBounds.Right := AButtonBounds.Left + GetButtonHeight;
      if I < ViewInfo.NavigatorButtonCount - 1 then
        AButtonLeftTopCorner.Y := AButtonBounds.Bottom + GetButtonsDistance(AButton,
          ViewInfo.NavigatorButtonInfos[I + 1].ButtonType);
    end
    else
    begin
      AButtonBounds.Right := AButtonBounds.Left + GetButtonWidth(AButton);
      AButtonBounds.Bottom := AButtonBounds.Top + GetButtonHeight;
      if I < ViewInfo.NavigatorButtonCount - 1 then
        AButtonLeftTopCorner.X := AButtonBounds.Right + GetButtonsDistance(AButton,
          ViewInfo.NavigatorButtonInfos[I + 1].ButtonType);
    end;
...
begin
  ...
  Result := Max(Result, IfThen(ViewInfo.HasTabCloseButtons, GetCloseButtonAreaHeight(-1)));
end;
...
begin
  ...
  if (ATabVisibleIndex <> -1) and TabViewInfo[ATabVisibleIndex].HasCloseButton then
    Inc(Result, GetCloseButtonAreaWidth(-1));
end;
...
begin
  ...
  begin
    ...
    if NavigatorButtons <> [] then
    begin
      ExtendRect(ExtendedTabsRect, RotateRect(Rect(0, ATabsContainerOffset.Top, 0, 0), TabPosition));
      ...
      ExtendRect(ExtendedTabsRect, RotateRect(ATabsContainerOffset, TabPosition));
    if TabPosition in [tpTop, tpBottom] then
...
begin
  ...
  AContentLeftTopCorner := GetButtonContentPosition(ARect, AType, AState);
...
begin
  for I := 0 to ViewInfo.NavigatorButtonCount - 1 do
    with ViewInfo.NavigatorButtonInfos[I] do
    begin
      ACanvas.SaveClipRegion;
      try
        ACanvas.SetClipRegion(TcxRegion.Create(Bounds), roIntersect);
        PaintButton(ACanvas, Bounds, State, ButtonType);
      finally
...
begin
  if ViewInfo.NavigatorButtons = [] then Exit;
  ViewInfo.InvalidateRect(FButtonsRect, False);
end;
...
begin
  ...
  begin
    ...
    if I < ViewInfo.NavigatorButtonCount - 1 then
      Inc(Result, GetButtonsDistance(AButton, ViewInfo.NavigatorButtonInfos[I + 1].ButtonType));
  end;
end;
...
function TcxPCButtonedPainter.UseLookAndFeelCloseButton: Boolean;
begin
  Result := True;
end;
...
function TcxPCExtraFlatPainter.GetTabCorrection(ATabVisibleIndex: Integer): TcxPCRectCorrection;
begin
...
begin
  Result := inherited GetPageClientRectOffset;
  if not ViewInfo.HideTabs and (ViewInfo.RowCount > 0) then
...
begin
  Result := TabViewInfo[ViewInfo.FocusedTabVisibleIndex].FullRect;
  CorrectRect(Result, AFocusRectCorrectionA[ViewInfo.TabPosition]);
end;
...
begin
  if NeedShowFrame and not ViewInfo.HideTabs and (ViewInfo.RowCount > 0) then
    Dec(R.Top);
end;
...
function TcxPCOffice11Painter.UseLookAndFeelCloseButton: Boolean;
begin
  Result := True;
end;
...
begin
  Result := inherited GetPageClientRectOffset;
  if not ViewInfo.HideTabs and (ViewInfo.RowCount > 0) and ViewInfo.IsTabsContainer then
    Inc(Result.Top, TabsContainerOffset);
  if not NeedShowFrame and not ViewInfo.HideTabs and (ViewInfo.RowCount > 0) then
  begin
    Inc(Result.Top);
    if ViewInfo.IsTabsOnBothSides then
      Inc(Result.Bottom);
...
begin
  ...
  if ViewInfo.IsTabsContainer then
    ExtendRect(Result, RotateRect(Rect(0, -TabsContainerOffset, 0, 0), ViewInfo.TabPosition));
end;
...
begin
  ...
  Result.Combine(TcxRegion.Create(ATabViewInfo.VisibleRect), roIntersect);
  if ATabViewInfo.IsMainTab then
    Result.Combine(TcxRegion.Create(GetTabUnderlineRect(ATabViewInfo)), roAdd);
end;
...
begin
  ...
  InflateRect(AFrameRect, 1, 1);
  if ViewInfo.IsTabsContainer then
    ExtendRect(AFrameRect, RotateRect(Rect(0, -TabsContainerOffset, 0, 0), ViewInfo.TabPosition));
  ACanvas.FrameRect(AFrameRect, GetFrameColor);
end;
