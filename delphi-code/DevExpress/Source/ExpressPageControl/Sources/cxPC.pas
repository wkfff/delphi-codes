...
begin
  if not Properties.MultiLine and ViewInfo.HasNavigatorButton(nbTopLeft) and
    ViewInfo.HasNavigatorButton(nbBottomRight) then
  begin
...
begin
  ...
  AButtonClick := (FHitTest.HitAtNavigatorButton and (FHitTest.HitObject = ViewInfo.PressedNavigatorButton)) or
    (HitTest.HitAtTabCloseButton and (FHitTest.HitObject = ViewInfo.PressedTabCloseButton));
  ViewInfo.PressedTabCloseButton := nil;
  ViewInfo.PressedNavigatorButton := nil;
  if AButtonClick then
    TcxPCCustomButtonViewInfo(FHitTest.HitObject).Click
  else
...
begin
  ...
  begin
    if not (ViewInfo.Painter.IsTabPressable or not Properties.ActivateFocusedTab) or
...
var
  ATabInfo: TcxTabViewInfo;
begin
  if ViewInfo.IsTabAccessible(ATabVisibleIndex) then
  begin
    ATabInfo := ViewInfo.TabsViewInfo[ATabVisibleIndex];
    ATabInfo.Select(ssCtrl in AShift);
    ATabInfo.SetFocus;
    ATabInfo.Click(AShift);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      if (APage.Enabled or AllowDisabledTabAccess) and Control.Visible and
        Control.Enabled and Control.Parent.CanFocus and APage.Enabled then
...
procedure TcxPCCustomPainter.DrawTabCloseButton(ACanvas: TcxCanvas; TabVisibleIndex: Integer);
  ...
  procedure InternalDrawTabCloseButton(ACanvas: TcxCanvas;
    const ARect: TRect; AState: TcxPCNavigatorButtonState);
  begin
    if UseLookAndFeelCloseButton then
      GetLookAndFeelPainter.DrawSmallCloseButton(
        ACanvas, ARect, NavigatorBtnStateToLookAndFeelBtnState[AState])
    else
      DoDrawTabCloseButton(ACanvas, ARect, AState)
  end;
...
begin
  ...
  if AAngle = ra0 then
    InternalDrawTabCloseButton(ACanvas, AButtonRect, AButtonInfo.State)
  else
  begin
    ABitmap := TcxBitmap.CreateSize(AButtonRect);
    try
      ABitmap.Rotate(ABackRotateAngle[AAngle]);
      InternalDrawTabCloseButton(ABitmap.cxCanvas, ABitmap.ClientRect, AButtonInfo.State);
...
function TcxPCTabCloseButtonViewInfo.GetState: TcxPCNavigatorButtonState;
begin
...
var
  ...
  begin
    ...
    if (FFirstTabVisibleIndex = -1) and (TabsViewInfo.ViewInfoCount > 0) then
      FFirstTabVisibleIndex := 0;
...
begin
  Result := False;
  if not Properties.MultiLine and (FFirstTabVisibleIndex >= 0) then
  begin
