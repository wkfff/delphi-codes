...
type
  ...
  private
    ...
    procedure ProcessMap(AWord: PWideChar; AIndex: Integer; AMapTable: TdxMapTable);
...
begin
  ...
  ProcessMap(PWideChar(Word), 0, AffixManager.MapTable);
end;
...
procedure TdxHunspellSuggestionBuilder.ProcessMap(AWord: PWideChar;
  AIndex: Integer; AMapTable: TdxMapTable);
var
  I, J, ALength : Integer;
  AChar: WideChar;
  AInMap: Boolean;
...
begin
  ...
  begin
    ...
    begin
      ...
      if StrScan(ACharSet, AChar) <> nil then
      begin
        AInMap := True;
        StrCopy(ACandidate, AWord);
        ALength := StrLen(ACharSet);
        for J := 0 to ALength - 1 do
        begin
          ACandidate[AIndex] := (ACharSet + J)^;
          ProcessMap(ACandidate, AIndex + 1, AMapTable);
        end;
      end;
    end;
    if not AInMap then
      ProcessMap(AWord, AIndex + 1, AMapTable);
  end;
end;
