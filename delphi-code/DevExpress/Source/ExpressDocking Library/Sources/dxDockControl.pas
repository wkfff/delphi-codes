...
uses
  ...
  Forms, Menus, ImgList, IniFiles, Contnrs,
...
const
  ...
  dxDockingDefaultStyle = {$IFDEF USENATIVELOOKANDFEELASDEFAULT}vsUseLookAndFeel{$ELSE}vsStandard{$ENDIF};
...
type
  ...
  protected
    ...
    procedure StartDocking(const pt: TPoint); virtual;
    procedure Docking(const pt: TPoint); virtual;
    procedure EndDocking(const pt: TPoint; Cancel: Boolean); virtual;
    ...
    function IsCaptionCloseButtonPoint(const pt: TPoint): Boolean;
    function IsCaptionHideButtonPoint(const pt: TPoint): Boolean;
    function IsCaptionMaximizeButtonPoint(const pt: TPoint): Boolean;
  ...
  protected
    ...
    property FirstVisibleTabIndex: Integer read GetFirstVisibleTabIndex;
  ...
  public
    ...
    function GetCaptionButtonsIndents: TRect; virtual;
  ...
  private
    ...
    function GetDefaultPainterClass: TdxDockControlPainterClass;
    function GetDockControl(Index: Integer): TdxCustomDockControl;
    function GetDockControlCount: Integer;
...
uses
  ...
  Math, cxGeometry, dxOffice11, dxHooks;
...
begin
  if not IsDesigning and (Message.HitTest = HTCAPTION) and not IsIconic(Handle) and
    (DockSite <> nil) and (doDblClickDocking in DockSite.ControllerOptions) then
...
begin
  ...
  FDockType := dtNone;
  FDockZones := TObjectList.Create;
  FDockingPoint := cxInvalidPoint;
  FImageIndex := -1;
  ManagerColor := True;
  ManagerFont := True;
  FResizeZones := TObjectList.Create;
  ParentColor := False;
  ParentFont := False;
...
procedure TdxCustomDockControl.StartDocking(const pt: TPoint);
var
  R: TRect;
begin
  if IsAncestor then
...
procedure TdxCustomDockControl.Docking(const pt: TPoint);
var
  AAccept: Boolean;
...
procedure TdxCustomDockControl.EndDocking(const pt: TPoint; Cancel: Boolean);
var
  ATargetZone: TdxZone;
begin
  try
...
begin
  ...
  begin
    ...
    if IsCaptionVertical then
      ...
      Result := Painter.GetCaptionAreaHeight + ABorderWidths.Top + ABorderWidths.Bottom;
  end;
end;
...
begin
  ...
  begin
    ...
    try
      NCPaintCaption(cxPaintCanvas.Canvas);
    finally
      cxPaintCanvas.EndPaint;
      ReleaseDC(Handle, DC);
    end;
...
function TdxCustomDockControl.IsCaptionCloseButtonPoint(const pt: TPoint): Boolean;
begin
  Result := HasCaptionCloseButton and PtInRect(CaptionCloseButtonRect, ClientToWindow(pt));
end;

function TdxCustomDockControl.IsCaptionHideButtonPoint(const pt: TPoint): Boolean;
begin
  Result := HasCaptionHideButton and PtInRect(CaptionHideButtonRect, ClientToWindow(pt));
end;

function TdxCustomDockControl.IsCaptionMaximizeButtonPoint(const pt: TPoint): Boolean;
begin
...
begin
  if not Visible or
    IsDesigning and (ParentDockControl <> nil) and PtInRect(GetGetTopMostDockControlDesignRect, SmallPointToPoint(Message.Pos)) then
...
begin
  ...
  if (ActiveChildIndex >= 0) and ((FirstVisibleTabIndex > ActiveChildIndex) or (ActiveChildIndex > LastVisibleTabIndex)) then
  begin
    NCChanged(True);
...
function TdxTabContainerDockSite.GetFirstVisibleTabIndex: Integer;
begin
  FFirstVisibleTabIndex := Min(FFirstVisibleTabIndex, ChildCount - 1);
...
function TdxDockControlPainter.GetCaptionButtonsIndents: TRect;
begin
  Result := cxNullRect;
end;
...
begin
  ...
  begin
    ...
    if IsActive then
      ...
      pts[0] := Point(ARect.Left, ARect.Bottom - 3);
  ...
  begin
    ...
    if IsActive then
      ...
      ACanvas.MoveTo(ARect.Left, ARect.Top + 2);
...
begin
  with Result do
    ...
    begin
      Left := ARect.Left;
      Top := ARect.Top;
      Right := Left + GetCaptionAreaHeight;
      Bottom := ARect.Bottom;
    end
    else
    begin
      Left := ARect.Left;
      Top := ARect.Top;
      Bottom := Top + GetCaptionAreaHeight;
      Right := ARect.Right;
    end;
end;
...
var
  R: TRect;
begin
  R := GetCaptionButtonsIndents;
  Result := Max(GetCaptionHeight, GetCaptionButtonSize + R.Top + R.Bottom);
end;
...
procedure dxDockWndProcHook(ACode, WParam, LParam: Integer; var AHookResult: Integer);
...
begin
  KillMessages(Handle, DXM_DOCK_DESTROYCONTROLS);
...
begin
  case Message.Msg of
    DXM_DOCK_DESTROYCONTROLS: DestroyControls;
...
function TdxDockingController.GetDefaultPainterClass: TdxDockControlPainterClass;
begin
  Result := {$IFDEF USENATIVELOOKANDFEELASDEFAULT}TdxDockControlXPPainter{$ELSE}TdxDockControlPainter{$ENDIF};
end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        KillMessages(0, DXM_DOCK_CHECKACTIVEDOCKCONTROL);
...
begin
  ...
  FViewStyle := dxDockingDefaultStyle;
...
const
  AStyles: array[TcxLookAndFeelKind] of TdxDockingViewStyle = (
    vsNET, vsStandard, vsNET, vsOffice11);
