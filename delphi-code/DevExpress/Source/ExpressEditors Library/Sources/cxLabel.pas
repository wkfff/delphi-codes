      ...
      begin
        ...
        Result.X := Round(
          (ARect.Left + ARect.Right) / 2 -
            cxRectWidth(ADrawingRect) / 2  * Cos(AAngle * PI / 180) -
            cxRectHeight(ADrawingRect) / 2 * Sin(AAngle * PI / 180));
        Result.Y := Round(
          (ARect.Top + ARect.Bottom)/2 +
            cxRectWidth(ADrawingRect) / 2  * Sin(AAngle * PI / 180) -
            cxRectHeight(ADrawingRect) / 2 * Cos(AAngle * PI / 180));
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      ATextFlags := PrepareTextFlag(Alignment.Horz, TcxAlignmentVert(Ord(Alignment.Vert)),
        (epoShowEndEllipsis in PaintOptions) and IsInplace, WordWrap, ShowAccelChar);
...
begin
  ...
  try
    ...
    if not IsInplace and Transparent then
