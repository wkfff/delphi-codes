...
uses
  ...
  Menus, RichEdit, StdCtrls, SysUtils,
  dxCore, dxMessages, cxClasses, cxContainer,
  cxControls, cxEdit, cxDrawTextUtils, cxGraphics, cxLookAndFeels, cxMemo,
  cxScrollbar, cxTextEdit, dxGDIPlusApi;
...
uses
  ...
  cxEditPaintUtils, cxEditUtils, cxExtEditConsts, cxVariants,
...
const
  ...
  begin
    ...
    Result.Right := MulDiv(Result.Right, TwipsPerInch, GetDeviceCaps(ADC, LOGPIXELSX));
    if ACalculateHeight then
      Result.Bottom := TwipsPerInch
    else
      Result.Bottom := MulDiv(Result.Bottom (*65535*), TwipsPerInch, GetDeviceCaps(ADC, LOGPIXELSY));
  end;
...
begin
  if not ACalculateHeight then
    FillRect(ADC, Rect(0, 0, cxRectWidth(ARect), cxRectHeight(ARect)), GetSolidBrush(AColor));
...
begin
  ...
  try
    if Edit.HandleAllocated then
      SendMessage(Edit.Handle, WM_SETTEXT, 0, Integer(PChar('')));

    if AContainer.ActiveProperties.MemoMode or not Edit.HandleAllocated then
      Edit.Text := dxVariantToString(Value)
    else
      LoadRichFromString(Edit.RichLines, dxVariantToString(Value));
  finally
...
begin
  ...
  try
    ...
    if FRichEdit.SelStart <> (ASelection.cpMax + dxCharCount(AStr)) then
...
begin
  ...
  {$IFDEF DELPHI12}
    ...
    if not VarIsNull(FSavedEditValue) then
...
begin
  if HandleAllocated and MemoMode then
    ...
    if (Container <> nil) and not Container.IsInplace then
      SetRichDefAttributes(Self, Font, Container.VisibleFont.Color);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      cxDrawRichEdit(ADC, Rect(0, 0, AEditSizeProperties.Width, 0), VarToStr(AEditValue),
