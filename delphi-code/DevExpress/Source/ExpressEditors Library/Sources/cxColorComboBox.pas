...
uses
  ...
  StdCtrls, Forms, Dialogs, cxClasses, cxControls, cxContainer, cxGraphics,
...
type
  ...
  protected
    ...
    function DoSelectCustomColor(AColor: TColor): Integer; virtual;
    procedure ClearEditValue; virtual;
    function DoOnSelectCustomColor(var AColor: TColor; out AColorDescription: string): Boolean;
...
uses
  dxCore, cxExtEditUtils, dxThemeConsts, dxThemeManager, dxUxTheme;
...
begin
  if IsVarEmpty(AEditValue) or not cxStrToColor(VarToStr(AEditValue), Result) then
    Result := DefaultColor;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      if AIndex > -1 then
      begin
        FMRUColors.InsertColor(0, AColor, Items[AIndex].Description);
        DeleteOverMRUColors;
        if Assigned(FOnAddedMRUColor) then FOnAddedMRUColor(Self);
      end
      else
...
begin
  ...
  begin
    AValid := cxStrToColor(VarToStr(AEditValue), AColor);
    if AValid then
    begin
...
var
  ...
  AIndex: Integer;
begin
  if cxStrToColor(ADisplayValue, AColor) then
  begin
    AIndex := ActiveProperties.GetIndexByColor(AColor);
    if AIndex = -1 then
      AIndex := DoSelectCustomColor(AColor);
  end
  else
...
function TcxCustomColorComboBox.DoSelectCustomColor(AColor: TColor): Integer;
var
  AColorDescription: string;
begin
  if (ActiveProperties.GetIndexByColor(AColor) = -1) and DoOnSelectCustomColor(AColor, AColorDescription) then
...
var
  AColor: TColor;
begin
  if cxStrToColor(Text, AColor) then
    DoSelectCustomColor(AColor);
  inherited;
end;
...
function TcxCustomColorComboBox.DoOnSelectCustomColor(var AColor: TColor; out AColorDescription: string): Boolean;
begin
  Result := True;
  AColorDescription := '';
  with Properties do
...
begin
  ...
  begin
    ...
    LookupData.SetCurrentKey(ActiveProperties.GetIndexByColor(ColorValue));
  end;
end;
...
begin
  ...
  LookupData.SetCurrentKey(DoSelectCustomColor(AColorProvided));
...
begin
  ...
  if (APrevKey = VK_RETURN) and not (ssAlt in Shift) and
      (ItemIndex = -1) and cxStrToColor(Text, AColor) then
    LookupData.SetCurrentKey(DoSelectCustomColor(AColor));
end;
