...
var
  FHintController: TcxHintController;
...
begin
  ...
  FHintController.RegisterHintStyleController(Self);
end;
...
var
  I: Integer;
begin
  FHintController.UnRegisterHintStyleController(Self);
...
var
  R: TRect;
begin
  FHintController.HintedControl := FindVCLWindow(Point(X, Y));
...
begin
  FHintController.HintedControl := nil;
...
begin
  Result := (FHintController.FControllerList.Count > 1) or not FGlobal;
end;
...
begin
  cxDrawText(Canvas.Handle, ACaption, ARect, DT_CALCRECT or DT_WORDBREAK or DT_NOPREFIX);
end;
...
var
  ...
  AIHint: IcxHint;
begin
  if Supports(FHintController.HintedControl, IcxHint, AIHint) then
    LoadPropertiesFromHintInterface(AIHint)
  else
  begin
    AController := FHintController.FindHintStyleController;
    if AController <> nil then
...
initialization
  FHintController := TcxHintController.Create(nil);
