...
uses
  Classes, SysUtils, Windows, Messages, Variants, Types,
...
type
  ...
  protected
    Hint: TCaption;
    HintAreaBounds: TRect;
    HintCheckerTimer: TcxTimer;
...
procedure cxDragMouseHook(ACode, WParam, LParam: Integer; var AHookResult: Integer);
begin
...
begin
  ...
  HintCheckerTimer := TcxTimer.Create(nil);
  HintCheckerTimer.Enabled := False;
  HintCheckerTimer.Interval := 100;
  HintCheckerTimer.OnTimer := HintCheckerTimerHandler;  
  ResetTimer;
end;
...
begin
  ...
  FreeAndNil(HintTimer);
  FreeAndNil(HintCheckerTimer);
  inherited Destroy;
end;
...
begin
  ...
  begin
    ...
    AControlScreenOrigin := cxPoint(PrevHitPoint.X, PrevHitPoint.Y);
    Result := AIntf.IsNeedHint(cxScreenCanvas, AControlScreenOrigin, Hint, HintIsMultiLine,
    ...
    begin
      ...
      HintRect := HintWindow.CalcHintRect(Screen.Width, Hint, nil);
      HintAreaBounds := AIntf.GetHintBounds;
      IntersectRect(HintAreaBounds, HintAreaBounds, Control.ClientBounds);
      AControlScreenOrigin := Control.ClientToScreen(cxNullPoint);
      if PrevElement is TcxEditCellViewInfo then
      begin
        Dec(AControlScreenOrigin.X);
        Dec(AControlScreenOrigin.Y);
      end;
      OffsetRect(HintAreaBounds, AControlScreenOrigin.X, AControlScreenOrigin.Y);
      HintRect := cxRectOffset(HintRect, HintAreaBounds.TopLeft);
      if AIsNeedOffsetHint then
        OffsetRect(HintRect, 0, cxGetCursorSize.cy);
    end;
    cxScreenCanvas.Dormant;
  end;
end;

procedure TcxHotTrackController.HintCheckerTimerHandler(Sender: TObject);
begin
  if not CanShowHint then
    InternalHideHint(nil);
end;
...
begin
  ...
  begin
    if (FPrevHintText = Hint) and cxRectIsEqual(HintRect, FPrevHintRect) then
      Exit;
  end;
  if (HintElement <> PrevElement) or (Hint <> FPrevHintText) then
  begin
    FPrevHintRect := HintRect;
    FPrevHintText := Hint;
    ...
    begin
      ...
      if Supports(TObject(HintWindow), IcxHintWindowHelper, AIntf) then
        AIntf.SetHintAreaBounds(HintAreaBounds);
      cxActivateHint(HintWindow, HintRect, Hint);
      InitTimer(AHintInfo.HideTimeout, (AHintInfo.HideTimeout > 0), InternalHideHint);
    end;
    HintCheckerTimer.Enabled := True;
  end;
end;
...
begin
  Result := Visible and
    ViewInfo.NeedShowHint(ACanvas, P, GetControl.ClientBounds,  AText, AIsMultiLine, ATextRect);
