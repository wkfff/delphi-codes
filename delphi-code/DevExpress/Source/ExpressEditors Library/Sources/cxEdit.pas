...
uses
  Windows, Messages, Variants, Forms, ComCtrls, Classes, Controls, Graphics,
  Menus, StdCtrls, SysUtils,
  cxClasses, cxContainer, cxControls, cxDataStorage, cxDataUtils, cxEditPaintUtils,
  cxGraphics, cxLookAndFeels, cxVariants, cxLookAndFeelPainters, dxCore;
...
//    property GradientButtons;
  ...
  TcxCustomEdit = class(TcxCustomEditContainer, IUnknown, IcxEditRepositoryItemListener,
...
var
  ...
  begin
    ...
    cxDrawImage(ACanvas.Handle, cxRectBounds(AGlyphPosition.X, AGlyphPosition.Y, AGlyph.Width, AGlyph.Height), AContentRect,
      AGlyph, nil, -1, EnabledImageDrawModeMap[not (AButtonViewInfo.Data.State = ebsDisabled)]);
  end;
  ...
  try
    ...
    begin
      ...
      Painter.DrawEditorButton(ACanvas, AContentRect, AKind,
        ...
        EditBtnPositionMap[AButtonViewInfo.Data.LeftAlignment]);
...
begin
  ...
  ABrushColor := 0;
  AButtonViewInfo := ButtonsInfo[AButtonVisibleIndex];
  ABoundsRect := AButtonViewInfo.Bounds;


  ABitmap := TcxBitmap.CreateSize(ABoundsRect, pf24bit);
  try
    cxBitBlt(ABitmap.Canvas.Handle, ACanvas.Handle, ABitmap.ClientRect, ABoundsRect.TopLeft, SRCCOPY);
    ABitmap.cxCanvas.WindowOrg := ABoundsRect.TopLeft;

    ABackgroundRect := ABoundsRect;
    DrawButtonBorder(ABitmap.cxCanvas, AButtonVisibleIndex, ABackgroundRect, AContentRect, APenColor, ABrushColor);

    if not IsRectEmpty(ABackgroundRect) then
      DrawButtonBackground(ABitmap.cxCanvas, AButtonVisibleIndex, ABackgroundRect, ABrushColor);
    if not IsRectEmpty(AContentRect) then
      DrawButtonContent(ABitmap.cxCanvas, AButtonVisibleIndex, AContentRect, APenColor, ABrushColor, NeedOffsetContent);

    ABitmap.cxCanvas.WindowOrg := cxNullPoint;
    ACanvas.Draw(ABoundsRect.Left, ABoundsRect.Top, ABitmap);
  finally
    ABitmap.Free;
  end;
end;
...
procedure TcxCustomEditViewData.CalculateButtonsViewInfo(ACanvas: TcxCanvas;
  ...
  begin
    ...
    begin
      ...
      with AButtonViewInfo do
      begin
        if not Enabled or not Properties.Buttons[ButtonIndex].Enabled then
...
begin
  ...
  begin
    ...
    Data.BackgroundPartiallyTransparent := Data.UseSkins or
      (Data.NativePart <> TC_NONE) and IsThemeBackgroundPartiallyTransparent(ATheme, Data.NativePart, Data.NativeState);
  end;
end;
...
begin
  ...
  begin
    ...
    if not AButton.Visible then
      Continue;
    if (AButton.Kind = bkGlyph) and IsGlyphAssigned(AButton.Glyph) then
...
begin
  if not (csDestroying in ComponentState) and (InnerControl <> nil) then
...
begin
  ...
  try
    ...
    AError := False;
    DoValidateDisplayValue(ADisplayValue, AErrorText, AError);
    Result := not AError;
    if AError then
      try
...
begin
  ...
  if not Result then
    case Message.Msg of
      ...
      WM_PASTE:
        begin
          if not InnerEdit.CanProcessClipboardMessages and DoEditing then
          begin
            KeyboardAction := True;
            try
              PasteFromClipboard;
...
begin
  with Properties do
    if Assigned(FOnButtonClick) then
      FOnButtonClick(Self, ViewInfo.ButtonsInfo[AButtonVisibleIndex].ButtonIndex);
...
procedure TcxCustomEdit.DoValidateDisplayValue(var ADisplayValue: TcxEditValue; var AErrorText: TCaption;
  var AError: Boolean);
begin
