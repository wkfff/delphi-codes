...
uses
  ...
  cxLookAndFeelPainters, cxControls, cxContainer, cxClasses;
...
type
  ...
  private
    ...
    procedure CalcButtonLayout(ACanvas: TCanvas; const AClient: TRect;
      ...
      ABiDiFlags: LongInt; AWordWrap: Boolean);
  protected
    ...
    procedure DrawButtonText(ACanvas: TCanvas; const ACaption: TCaption;
      ...
      ANativeStyle: Boolean; AWordWrap: Boolean; ASkinsUsed: Boolean = False); virtual;

    function CanDrawCaption: Boolean;
    function CanDrawGlyph: Boolean;
  public
    constructor Create(AButton: TcxCustomButton); virtual;
    destructor Destroy; override;
    procedure Draw(ACanvas: TCanvas; const AClient: TRect; const AOffset: TPoint;
      ...
      AState: TcxButtonState ; ABiDiFlags: LongInt; ANativeStyle: Boolean;
      AWordWrap: Boolean; ASkinsUsed: Boolean = False); virtual;
  ...
  TcxButtonInternalState = set of (bisDown, bisHot, bisPressed, bisMenuButtonPressed,
    bisMenuVisible, bisAccelExecute);
...
// for navigation
{$IFDEF DELPHI12}
  TcxCustomButton = class(TCustomButton, IdxSkinSupport, IcxMouseTrackingCaller, IcxLookAndFeelContainer)
{$ELSE}
  TcxCustomButton = class(TButton, IdxSkinSupport, IcxMouseTrackingCaller, IcxLookAndFeelContainer)
{$ENDIF}
  private
    FAutoSize: Boolean;
...
uses
  dxUxTheme, cxDWMApi, CommCtrl, dxThemeConsts,
...
constructor TcxButtonGlyph.Create(AButton: TcxCustomButton);
var
  I: TButtonState;
begin
  inherited Create;
...
function TcxButtonGlyph.CreateButtonGlyph(AState: TcxButtonState): Integer;
  ...
  begin
    ...
    begin
      ...
      TcxImageList.GetImageInfo(ImageList, ImageIndex, AImage, AMask);
  end;
  ...
  begin
    ...
    begin
      ...
      if ALayout in [blGlyphLeft, blGlyphRight] then
        TextBounds.Right := TextBounds.Right - GetImageSize.cx - ASpacing;
...
procedure TcxButtonGlyph.DrawButtonText(ACanvas: TCanvas; const ACaption: TCaption;
  ATextBounds: TRect; AState: TcxButtonState ; ABiDiFlags: LongInt;
  ANativeStyle: Boolean; AWordWrap: Boolean; ASkinsUsed: Boolean = False);
...
var
  ...
  AFontColor: TColor;
begin
  if not CanDrawCaption then Exit;
  ABrushStyle := ACanvas.Brush.Style;
  try
    ACanvas.Brush.Style := bsClear;
    if (AState = cxbsDisabled) and not ASkinsUsed then
    begin
...
function TcxButtonGlyph.CanDrawCaption: Boolean;
begin
  Result := (FButton.PaintStyle in [bpsDefault, bpsCaption]) and (Length(FButton.Caption) > 0);
end;

function TcxButtonGlyph.CanDrawGlyph: Boolean;
begin
  Result := (FButton.PaintStyle in [bpsDefault, bpsGlyph]) and ImageInfo.IsImageAssigned;
end;
...
begin
  ...
  if FColors.GetTextColorByState(AState) = clDefault then
    FCanvas.Font.Color := Painter.ButtonSymbolColor(AState, FCanvas.Font.Color)
  else
...
begin
  ...
  try
    ...
    Result.cy := MulDiv(ACanvas.TextHeight(dxMeasurePattern), 14, 8);
  finally
    ACanvas.Free;
  end;
end;
...
begin
  ...
  begin
    if PtInRect(GetMenuButtonBounds, P) then
      Include(FInternalState, bisMenuButtonPressed)
    else
      Include(FInternalState, bisPressed);
  end;
end;
...
begin
  if PtInRect(GetMenuButtonBounds, P) and not (bisMenuVisible in InternalState) and
      not FIsDropDownMenuClosing then
    DoDropDownMenu
end;
...
begin
  ...
  else if (bisHot in InternalState) and CanHotTrack then Result := cxbsHot
...
begin
  AExecuteClick := (bisPressed in InternalState) or
    ([bisPressed, bisMenuButtonPressed] * InternalState = []);
  if (Message.NotifyCode <> BN_CLICKED) or (Kind <> cxbkDropDownButton) or AExecuteClick then
    inherited;
end;
...
begin
  Result := ClientRect;
  ABorderSize := Painter.ButtonBorderSize(AState);
...
procedure TcxCustomButton.InternalPaint;
  ...
  begin
    if (SpeedButtonOptions.Flat or SpeedButtonOptions.Transparent) and not CanBeFocused then
    begin
      if NeedDrawBorder(AState) then
      begin
        Painter.DrawButtonBorder(FCanvas, R, AState);
        InflateRect(R, -Painter.ButtonBorderSize(AState), -Painter.ButtonBorderSize(AState));
      end;
      ...
      try
        ...
        if not SpeedButtonOptions.Transparent or ((AState <> cxbsDisabled) and
          ...
          ((bisHot in InternalState) and Painter.IsButtonHotTrack))
        then
          Painter.DrawButton(FCanvas, R, '', AState, False, AColor, FCanvas.Font.Color, WordWrap, True)
        else
      ...
      Painter.DrawButton(FCanvas, R, '', AState, True, AColor, FCanvas.Font.Color, WordWrap);
  end;

  procedure PrepareBackground(const R: TRect);
  var
    ATheme: TdxTheme;
  begin
...
begin
  ...
  try
    ...
    FGlyph.Draw(FControlCanvas, GetContentRect, AOffset, Caption, FLayout,
      ...
      Painter.LookAndFeelStyle = lfsNative, WordWrap, Assigned(LookAndFeel.SkinPainter));
  finally
    FCanvas.RestoreClipRegion;
  end;
  ...
  begin
    ...
    Painter.DrawScrollBarArrow(FCanvas, ATempRect, AButtonMenuState, adDown);
  end;

  if CanFocus then
    if Focused and ShowFocusRect and not (bisMenuVisible in InternalState) then
      FCanvas.DrawFocusRect(Painter.ButtonFocusRect(FCanvas, R));
end;
...
begin
  Result := not StandardButton and Painter.IsButtonHotTrack and Enabled;
end;
...
begin
  AExecuteClick := not CanBeFocused and (Button = mbLeft) and (bisHot in InternalState) and
    ((bisPressed in InternalState) or ((bisMenuButtonPressed in InternalState) and (Kind = cxbkDropDown)));
...
procedure TcxCustomButton.DrawItem(const DrawItemStruct: TDrawItemStruct);
  ...
  begin
    ...
    try
      ABitmap.Canvas.Lock;
      try
        FCanvas.Canvas.Handle := ABitmap.Canvas.Handle;
        InternalPaint;
        cxBitBlt(DrawItemStruct.hDC, FCanvas.Canvas.Handle, ClientRect, cxNullPoint, SRCCOPY);
...
begin
  ...
  begin
    FIsPaintPressed := ((itemState and ODS_SELECTED <> 0) and not (bisMenuButtonPressed in InternalState)) or
      ((bisMenuVisible in InternalState) and (Kind = cxbkDropDown)) or
...
begin
  inherited;
  if not (csDestroying in ComponentState) and HandleAllocated and SpeedButtonOptions.Transparent and not CanBeFocused then
    Invalidate;
end;
...
begin
  Msg.Message := 0;
  FIsDropDownMenuClosing := (bisMenuButtonPressed in FInternalState) or
    ((PeekMessage(Msg, Handle, WM_LBUTTONDOWN, WM_LBUTTONDOWN, PM_NOREMOVE) or PeekMessage(Msg, Handle, WM_LBUTTONDBLCLK, WM_LBUTTONDBLCLK, PM_NOREMOVE)) and
