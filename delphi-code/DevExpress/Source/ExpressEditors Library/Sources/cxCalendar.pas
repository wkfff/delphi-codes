...
type
  ...
  published
    property Color;
    property MinuteDotColor: TColor read FMinuteDotColor write SetMinuteDotColor default clWindow;
...
uses
  Math,
  dxCore, dxOffice11, dxThemeConsts, dxThemeManager, dxUxTheme,
...
begin
  ...
  {$IFNDEF DELPHI7}
    (Integer(AValue) = NullDate); //Delphi6 variants support
  {$ELSE}
    (AValue = NullDate);
  {$ENDIF}
end;
...
begin
  ...
  APrevDate := CalendarTable.AddMonths(FCurrentDate, TopMonthDelta);
  for I := TopMonthDelta + 1 to 0 do
    if APrevDate = CalendarTable.AddMonths(FCurrentDate, I) then
      Inc(FTopMonthDelta);
...
begin
  ...
  for I := dxGetStartOfWeek to dxGetStartOfWeek + 6 do
  begin
...
procedure TcxCustomCalendar.DrawDayNumbers(ACanvas: TcxCanvas; const R: TRect);
  ...
  function GetDayTextColor(ASelected, AToday: Boolean;
    const ADate, AFirstDate, ALastDate: Double): TColor;
  begin
    if ASelected then
...
var
  ADate, ACurDate, ALastDate: Double;
  ASelected, AToday: Boolean;
  ASize: TSize;
...
begin
  ...
  begin
    ...
    begin
      ...
      ASelected := (ADate = FSelectDate) or ((FSelectDate = NullDate) and (ADate = ACurDate));
      AToday := (ADate = ACurDate) or (ACurDate = NullDate);
      ASideRect := ATextRect;
      ...
      if ASelected then
        ACanvas.Brush.Color := LookAndFeelPainter.DefaultDateNavigatorSelectionColor
      else
        ACanvas.Brush.Color := BackgroundColor;
...
begin
  ...
  FButtonsHeight := MulDiv(Canvas.TextHeight(dxMeasurePattern), 20, 13) + 1;
  if Kind = ckDateTime then
  begin
