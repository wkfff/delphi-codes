...
uses
  Windows, Variants, Classes, Controls, Forms, Graphics, ImgList, StdCtrls, SysUtils,
...
procedure DrawCustomEdit(ACanvas: TcxCanvas; AViewInfo: TcxCustomEditViewInfo;
  ...
  begin
    with AViewInfo do
    begin
      if BorderStyle = ebsNone then Exit;
      Painter.DrawContainerBorder(ACanvas, R, TcxContainerBorderStyle(BorderStyle),
        ABorderWidth, BorderColor, Edges);
    end;
  end;
  ...
  begin
    ...
    begin
      R := BorderRect;
      ABorderWidth := Painter.GetContainerBorderWidth(TcxContainerBorderStyle(BorderStyle));
  ...
  begin
    ...
    begin
      if AViewInfo.NativeStyle then
        AViewInfo.DrawNativeStyleEditBackground(
          ACanvas, ADrawBackground, ABackgroundStyle, ABackgroundBrush)
      else
