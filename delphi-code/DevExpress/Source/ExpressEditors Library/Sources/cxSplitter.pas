...
type
  ...
  private
    ...
    procedure CalcSplitSize(X, Y: Integer; var ANewSize, ASplit: Integer;
...
begin
  case SplitterDirection of
    cxsdLeftToRight, cxsdRightToLeft: FRect := cxRectTransform(ARect, 0, 1, 0, -1);
    cxsdTopToBottom, cxsdBottomToTop: FRect := cxRectTransform(ARect, 1, 0, -1, 0);
  end;
...
begin
  ...
  begin
    ...
    begin
      case Value of
        ssOpened: InternalOpenSplitter;
        ssClosed: InternalCloseSplitter;
      end;
    end
    else
      FState := Value;
      ...
      begin
         ...
         begin
           ...
           case Result.Align of
             alLeft: if (R.Right = R.Left) and (R.Left = Width) then Exit;
             alRight: if (R.Right = R.Left) and (R.Right = Left) then Exit;
             alTop: if (R.Bottom = R.Top) and (R.Top = Height) then Exit;
             alBottom: if (R.Bottom = R.Top) and (R.Bottom = Top) then Exit;
           end;
         end;
      end;
    end;
...
procedure TcxCustomSplitter.CalcSplitSize(X, Y: Integer;
  var ANewSize, ASplit: Integer; ACorrectWithMaxMin: Boolean = True);
var
  S: Integer;
begin
  ...
  S := 0;
  case Align of
    alLeft: S := FControl.Width + ASplit;
    alRight: S := FControl.Width - ASplit;
    alTop: S := FControl.Height + ASplit;
    alBottom: S := FControl.Height - ASplit;
  end;
  ANewSize := S;
  ...
  case FSplitterState of
    sstHotZoneClick:
      ...
      begin
        ...
        case FState of
          ssClosed: InternalOpenSplitter;
          ssOpened: InternalCloseSplitter;
        end;
      end;
    sstResizing:
      begin
        ...
        UpdateStateAfterMoving(ANewSize);
        UpdateControlSize;
        if (ANewSize >= 0) then
          RecalcLastPosition;
      end;
...
procedure TcxCustomSplitter.CorrectSelfPosition;
  ...
  {$IFDEF DELPHI10}
    if AlignWithMargins then
      case Align of
        alLeft: Result := Margins.Left;
        alTop: Result := Margins.Top;
        alRight: Result := Margins.Right;
        alBottom: Result := Margins.Bottom;
      end;
  {$ENDIF}
  end;
