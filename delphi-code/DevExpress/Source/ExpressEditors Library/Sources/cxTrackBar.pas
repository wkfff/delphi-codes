...
var
  ...
  AThumbHalfSize: Integer;
begin
  AThumbHalfSize := AViewInfo.ThumbSize div 2;
  if Properties.Orientation = tboHorizontal then
  begin
    ACurrentTickOffset := AViewInfo.TrackRect.Left + AThumbHalfSize +
      Round(AViewInfo.TickOffset * (AViewInfo.Position - Properties.Min));
    AViewInfo.ThumbRect.Left := ACurrentTickOffset - AThumbHalfSize;
  ...
  begin
    ACurrentTickOffset := AViewInfo.TrackRect.Top + AThumbHalfSize +
      Round(AViewInfo.TickOffset * (AViewInfo.Position - Properties.Min));
    AViewInfo.ThumbRect.Top := ACurrentTickOffset - AThumbHalfSize;
    AViewInfo.ThumbRect.Bottom := AViewInfo.ThumbRect.Top + AViewInfo.ThumbSize;
    AViewInfo.ThumbRect.Left := AViewInfo.TrackRect.Left +
      (AViewInfo.TrackSize - AViewInfo.ThumbLargeSize) div 2;
...
begin
  ...
  if (tbmpSliding in ViewInfo.MouseStates) and not (ssLeft in InternalGetShiftState) then
  begin
...
begin
  ...
  if (tbmpSliding in ViewInfo.MouseStates) and (ssLeft in InternalGetShiftState) then
  begin
    ...
    begin
      if (ssShift in InternalGetShiftState) and (FSlideState = tbksIncludeSelection) then
