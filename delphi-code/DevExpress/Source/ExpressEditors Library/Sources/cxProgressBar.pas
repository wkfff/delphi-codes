...
procedure TcxCustomProgressBarViewInfo.Paint(ACanvas: TcxCanvas);
  ...
  begin
    Result := ChangedBounds or IsDoubleBuffered or (ShowText and (GetDrawText <> ''));
  end;
...
begin
  ...
  try
    ...
    cxBitBlt(ACanvas.Canvas.Handle, FUsualBitmap.cxCanvas.Handle, BarRect, cxPoint(ADrawDelta, ADrawDelta), SRCCOPY);
  finally
...
begin
  ADrawDelta := GetDrawDelta;
  FUsualBitmap.SetSize(cxRectWidth(BarRect) + ADrawDelta, cxRectHeight(BarRect) + ADrawDelta);
  ChangedBoundsBarRect := False;
end;
...
begin
  if NativeStyle or IsInplace or UseSkins then
    Result := 0
  else
    Result := 2;
end;
...
begin
  ...
  OffsetRect(ATextRect,
    GetDrawDelta + (cxRectWidth(ABarRect) - cxRectWidth(ATextRect)) div 2,
    GetDrawDelta + (cxRectHeight(ABarRect) - cxRectHeight(ATextRect)) div 2);
...
begin
  if FOrientation = cxorHorizontal then
    Result := Trunc(cxRectWidth(Bounds) / GetMaxMinDiff * GetRelativePosition)
  else
...
begin
  if FOrientation = cxorHorizontal then
    ...
    Result := cxRectHeight(Bounds) - Trunc(cxRectHeight(Bounds) / GetMaxMinDiff * GetRelativePosition);
end;
...
begin
  ...
  begin
    ...
    ABarRect := ABounds;
    if IsInplace then
      FBounds := cxRectTransform(FBounds, (AViewInfo.BorderWidth + 2), (AViewInfo.BorderWidth + 2),
        -(AViewInfo.BorderWidth + 2), -(AViewInfo.BorderWidth + 2))
    else
      if not (Properties.BarStyle in [cxbsAnimation, cxbsAnimationLEDs]) then
        FBounds := cxRectTransform(FBounds, (AViewInfo.BorderWidth + 3), (AViewInfo.BorderWidth + 3),
          -(AViewInfo.BorderWidth + 2), -(AViewInfo.BorderWidth + 2))
      else
        FBounds := cxRectTransform(FBounds, (AViewInfo.BorderWidth + 1), (AViewInfo.BorderWidth + 1),
  ...
  begin
    if FViewInfo.FOrientation = cxorHorizontal then
      AProgressBarRect.Right := FBounds.Left +
        CalculateDelta(FViewInfo.Position - FViewInfo.Min, cxRectWidth(FBounds),
          FViewInfo.MaxMinDiff)
    else
      AProgressBarRect.Top := FBounds.Bottom -
        CalculateDelta((FViewInfo.Position - FViewInfo.Min), cxRectHeight(FBounds),
          FViewInfo.MaxMinDiff);
  end;
  ...
  begin
    ...
    begin
      AOverloadBarRect.Left := FBounds.Left +
        CalculateDelta(FViewInfo.RelativeOverloadValue, cxRectWidth(FBounds),
          FViewInfo.MaxMinDiff);
    ...
    begin
      AOverloadBarRect.Top := AOverloadBarRect.Bottom -
        CalculateDelta(FViewInfo.RelativePosition, cxRectHeight(FBounds),
          FViewInfo.MaxMinDiff);
      AOverloadBarRect.Bottom := AOverloadBarRect.Bottom -
        CalculateDelta(FViewInfo.RelativeOverloadValue, cxRectHeight(FBounds),
          FViewInfo.MaxMinDiff);
  ...
  begin
    ...
    begin
      APeakBarRect.Left := FBounds.Left +
        CalculateDelta(FViewInfo.RelativePeakValue, cxRectWidth(FBounds),
        FViewInfo.MaxMinDiff);
    ...
    begin
      APeakBarRect.Bottom := FBounds.Bottom -
        CalculateDelta(FViewInfo.RelativePeakValue, cxRectHeight(FBounds),
          FViewInfo.MaxMinDiff);
