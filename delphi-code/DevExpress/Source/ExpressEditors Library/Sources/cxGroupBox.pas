...
uses
  ...
  cxEditPaintUtils, cxEdit, cxTextEdit, cxCheckBox, cxLookAndFeelPainters;
...
type
  ...
  private
    ...
    function GetCaptionSize(ACanvas: TcxCanvas): TSize;
  ...
  protected
    ...
    procedure CalcRectsForPanel(ACanvas: TcxCanvas);
    procedure CalcRectsForStandardPainter(ACanvas: TcxCanvas; AShadowWidth: Integer);
    procedure CalcRectsForSkinPainter(ACanvas: TcxCanvas; AHasNonClientArea: Boolean);
...
procedure TcxGroupBoxViewInfo.CalcRectsForPanel(ACanvas: TcxCanvas);
...
procedure TcxGroupBoxViewInfo.CalcRectsForStandardPainter(ACanvas: TcxCanvas; AShadowWidth: Integer);
  ...
  begin
    ...
    begin
      with GetCaptionSize(ACanvas) do
...
procedure TcxGroupBoxViewInfo.CalcRectsForSkinPainter(ACanvas: TcxCanvas; AHasNonClientArea: Boolean);
  ...
  procedure CalculateCaptionRect(var R: TRect; const AIndent: TRect; const ATextSize: TSize);
  begin
    case CaptionPosition of
      cxgpBottom:
...
begin
  ...
  ATextSize := GetCaptionSize(ACanvas);
  ATextSize.cy := Max(ATextSize.cy, ACanvas.TextHeight(dxMeasurePattern));
  CalculateCaptionRect(CaptionRect, ARect, ATextSize);
  CalculateBorderRect(BorderRect, ARect);
  CalculateTextRect(Painter.GroupBoxBorderSize(True, CaptionPosition), ATextSize.cx);
  AdjustCaptionRect;
end;
...
function TcxGroupBoxViewInfo.GetCaptionSize(ACanvas: TcxCanvas): TSize;
begin
  if Edit.Caption = '' then
    Result := cxNullSize
  else
    with GetCaptionRect(ACanvas, Rect(0, 0, 1, 1)) do
      Result := cxSize(Right, Bottom);
end;
...
begin
  Result := cxGroupBoxAlignment2GroupBoxCaption(Edit.Alignment);
end;
...
begin
  ...
  if not IsPanelStyle then
    case CaptionPosition of
      cxgpTop:
        Result.Top := ACanvas.TextHeight(dxMeasurePattern) div 2;
      cxgpBottom:
        Dec(Result.Bottom, ACanvas.TextHeight(dxMeasurePattern) div 2);
      cxgpLeft:
        Result.Left := ACanvas.TextHeight(dxMeasurePattern) div 2;
      cxgpRight:
        Dec(Result.Right, ACanvas.TextHeight(dxMeasurePattern) div 2);
    end;
end;
...
begin
  ...
  begin
    AHeaderSideClientExtent := cxTextHeight(Edit.FCaptionFont) + Result.Top;
...
begin
  Edit.AdjustCanvasFontSettings(ACanvas);
  if IsPanelStyle then
    AEditViewInfo.CalcRectsForPanel(ACanvas)
  else
    if Style.LookAndFeel.SkinPainter = nil then
      AEditViewInfo.CalcRectsForStandardPainter(ACanvas, GetShadowWidth)
    else
      AEditViewInfo.CalcRectsForSkinPainter(ACanvas, HasNonClientArea);
end;
  ...
  case Message.Msg of
    ...
    WM_NCCALCSIZE:
      ...
      begin
        ...
        R := cxRectContent(R, TcxGroupBoxViewInfo(ViewInfo).GetCaptionRectIndent);
        TWMNCCalcSize(Message).CalcSize_Params^.rgrc[0] := R;
      end;
    else
      if (Message.Msg = DXM_UPDATENONCLIENTAREA) and IsNonClientAreaSupported then
      begin
...
begin
  ...
  if not ViewInfo.UseSkins and IsNativeStyle then
    APainter := cxLookAndFeelPaintersManager.GetPainter(lfsNative);
  if ViewInfo.UseSkins or IsNativeStyle then
  begin
    if IsPanelStyle then
