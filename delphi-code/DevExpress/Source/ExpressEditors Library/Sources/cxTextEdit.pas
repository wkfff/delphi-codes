...
uses
  ...
  dxCore, dxMessages, cxClasses, cxContainer, cxControls, cxDataUtils, cxFormats,
  cxGraphics, cxVariants, cxFilterControlUtils, cxEdit, cxDrawTextUtils;
...
uses
  ...
  cxGeometry, cxEditConsts, cxEditUtils, dxThemeConsts, dxThemeManager, cxDWMApi;
...
begin
  ...
  begin
    ...
    AMeasureTextHeight := ACanvas.TextHeight(dxMeasurePattern);
...
function GetTextEditContentSize(ACanvas: TcxCanvas; AViewData: TcxCustomEditViewData;
  ...
  begin
    ...
    begin
      Result.cy := ACanvas.TextHeight(dxMeasurePattern) * ALineCount;
    ...
    begin
      ...
      begin
        ...
        if (epoAllowZeroHeight in AViewData.PaintOptions) and (Length(AText) = 0) then
          ...
          Result.cy := ACanvas.TextHeight(dxMeasurePattern);
      end
      else
      begin
        Result.cx := AEditSizeProperties.Width;
        ...
        begin
          ...
          Result.cy := ARowCount * ACanvas.TextHeight(dxMeasurePattern);
...
begin
  Result := inherited GetSupportedOperations + [esoAutoHeight, esoEditing,
    esoFiltering, esoHorzAlignment, esoIncSearch, esoSorting, esoEditingAutoHeight];
end;
    ...
    begin
      ...
      begin
        ...
        if TryStrToBcd(DisplayValue, V) then
          DisplayValue := BcdToStrF(V, ffFixed, APrecision,
...
begin
  ...
  begin
    ...
    if VarIsArray(AEditValue) and not (VarArrayDimCount(AEditValue) = 1) then
      ...
      DisplayValue := dxVariantToString(AEditValue);
end;
...
begin
  if HasInnerEdit and NeedInnerEditRecreateWnd then
    _TWinControlAccess._RecreateWnd(InnerControl);
  inherited;
end;
...
begin
  case Message.Msg of
    ...
    WM_UNDO, EM_UNDO:
      ...
      begin
        KillMessages(InnerControl.Handle, DXM_SPELL_REDRAWMISSPELLINGS);
        DoDrawMisspellings;
      end;
  end;
end;
