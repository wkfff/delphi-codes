...
function GraphicToPNGImage(const AClientRect, ADrawRect: TRect;
  AGraphic: TGraphic; AUseAlphaChannel: Boolean): TdxPNGImage;
...
begin
  ...
  B := cxCreateBitmap(cxRectSize(AClientRect), pf32Bit);
  try
    B.Canvas.Brush.Color := clWhite;
    B.Canvas.FillRect(AClientRect);
    B.Canvas.StretchDraw(ADrawRect, AGraphic);
    ABits := TdxGPImage.GetBitmapBits(B);
    for I := 0 to Length(ABits) - 1 do
      ABits[I].rgbReserved := Transparency[PtInRect(ADrawRect, Point(I mod B.Width, I div B.Width))];
    Result.Handle := TdxGPImage.CreateFromPattern(B.Width, B.Height, ABits, True);
  finally
    B.Free;
  end;
end;
