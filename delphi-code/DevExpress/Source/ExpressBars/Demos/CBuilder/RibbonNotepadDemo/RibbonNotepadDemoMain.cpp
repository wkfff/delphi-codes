...
//---------------------------------------------------------------------------
void TRibbonDemoMainForm::UpdateRibbonDemoStyle(TRibbonDemoStyle AStyle)
{
...
void __fastcall TRibbonDemoMainForm::pbFrameBackgroundPaint(TObject *Sender)
{
	TPaintBox *PaintBox = (TPaintBox *)Sender;

	dxGpFillRect(PaintBox->Canvas->Handle, PaintBox->ClientRect, clWhite, 200);
	ExcludeClipRect(PaintBox->Canvas->Handle, 1, 1, PaintBox->Width - 1, PaintBox->Height - 1);
	dxGpFillRect(PaintBox->Canvas->Handle, PaintBox->ClientRect, clWhite);
}

void __fastcall TRibbonDemoMainForm::pbSeparatorPaint(TObject *Sender)
{
   TPaintBox *PaintBox = (TPaintBox *)Sender;
   Ribbon->ColorScheme->DrawMenuSeparatorHorz(PaintBox->Canvas->Handle, PaintBox->ClientRect);
}

void __fastcall TRibbonDemoMainForm::bvtsHelpResize(TObject *Sender)
{
  imLogo->Left = imLogo->Parent->ClientWidth - imLogo->Width - Ceil(DPIRatio() * 10);
}

void __fastcall TRibbonDemoMainForm::bvtsAboutResize(TObject *Sender)
{
  Integer AOffset;
  AOffset = Ceil(DPIRatio() * 8);
...
void __fastcall TRibbonDemoMainForm::dxBarButtonOptionsClick(TObject *Sender)
{
  ...
  if (ExecuteRibbonDemoOptions(&AColorSchemeName, &AScreenTipOptions, &AStyle))
  {
