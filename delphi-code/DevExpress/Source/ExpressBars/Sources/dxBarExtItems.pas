...
uses
  Math, SysUtils, ActiveX, Printers, dxBarStrs,
...
begin
  Result := DayOfWeek(ADate) - dxGetStartOfWeek - 1;
...
var
  ...
  begin
    ...
    begin
      ...
      S := dxFormatSettings.LongMonthNames[Month] + ' ' + IntToStr(Year);
      ...
      begin
        ...
        J := dxGetStartOfWeek + 1 + I;
        if J > 7 then Dec(J, 7);
        if cxGetWritingDirection(Font.Charset, dxFormatSettings.ShortDayNames[1]) = coRightToLeft then
          S := AnsiLastChar(dxFormatSettings.ShortDayNames[J])
        else
          S := WideString(dxFormatSettings.ShortDayNames[J])[1];
        GetTextExtentPoint32(Handle, PChar(S), Length(S), Size);
        with TextR do
