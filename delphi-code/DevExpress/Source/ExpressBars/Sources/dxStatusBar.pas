...
type
  ...
  public
    ...
    class function PanelContentExtents(APanel: TdxStatusBarPanel; AHasBorders: Boolean): TRect; virtual;
  ...
  { TdxStatusBarItemViewInfo }

  TdxStatusBarPanelViewInfo = class(TObject)
  private
    FBounds: TRect;
  ...
  public
    constructor Create(APanel: TdxStatusBarPanel); virtual;
    //
...
begin
  ...
  begin
    ...
    Inc(Result, cxMarginsWidth(Painter.PanelContentExtents(Owner,
      Painter.GetPanelBevel(Owner) <> dxpbNone)));
  end
  else
    Result := 20;
end;
  ...
  begin
    ...
    begin
      ...
      R.Right := R.Left + FKeyInfos[I].GetRectWidth(ACanvas) +
        cxMarginsWidth(APainter.PanelContentExtents(Owner, not FullRect));
      ARect := R;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Inc(Result, cxMarginsWidth(Painter.PanelContentExtents(Owner, not FullRect)));
        // key separator
        if I <> ALastKeyIndex then
...
procedure TdxStatusBarContainerPanelStyle.BoundsChanged(const ABounds: TRect);
  ...
  begin
    ...
    AControl.SetBounds(0, 0, Container.Width, Container.Height);
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      AAutoWidthObject.AvailableWidth := cxRectWidth(ABounds) +
        cxMarginsWidth(StatusBar.Painter.ContentExtents(StatusBar));
...
begin
  ...
  begin
    if StatusBar.Panels[I].Visible then
      FPanelsViewInfos.Add(TdxStatusBarPanelViewInfo.Create(StatusBar.Panels[I]));
  end;
end;
...
procedure TdxCustomStatusBar.Paint;
  ...
  procedure DrawPanel(APanelViewInfo: TdxStatusBarPanelViewInfo);
  begin
    Painter.DrawPanel(Self, APanelViewInfo.Panel, Canvas, APanelViewInfo.Bounds);
    if APanelViewInfo.HasSeparator then
      Painter.DrawPanelSeparator(Self, Canvas, APanelViewInfo.SeparatorRect);
  end;
  ...
  begin
    ...
    begin
      for I := 0 to ViewInfo.PanelCount - 1 do
        DrawPanel(ViewInfo.PanelViewInfo[I]);
    end
    else
  ...
  begin
    if SizeGripAllocated then
      Painter.DrawSizeGrip(Self, Canvas, R,
        (ViewInfo.PanelCount <> 0) and ViewInfo.SizeGripOvelapped); // <- ExcludeClipRect
  end;
...
begin
  Result := cxRectContent(ClientBounds, Painter.ContentExtents(Self));
end;
...
class function TdxStatusBarPainter.PanelContentExtents(
  APanel: TdxStatusBarPanel; AHasBorders: Boolean): TRect;
begin
  if AHasBorders then
    Result := Rect(1, 0, 1, 0)
  else
