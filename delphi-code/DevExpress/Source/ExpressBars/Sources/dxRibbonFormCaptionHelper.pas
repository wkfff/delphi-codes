...
uses
  ...
  dxCore, dxMessages, cxClasses, cxGraphics, cxControls, dxRibbonSkins;
...
type
  ...
  public
    ...
    property IsApplicationMenuDropped: Boolean read GetIsApplicationMenuDropped;
...
function IsShowMinimizedOnDesktop(H: THandle): Boolean;
begin
  Result := (GetWindowLong(H, GWL_STYLE) and WS_CHILDWINDOW <> 0) or
...
begin
  Result := IRibbonFormNonClientDraw.GetRibbonFormCaptionAreaExtention;
end;
...
function TdxRibbonFormCaptionHelper.GetIsApplicationMenuDropped: Boolean;
begin
  Result := IRibbonFormNonClientDraw.GetIsApplicationMenuDropped;
end;
...
begin
  Result := TestWinStyle(WS_SYSMENU) and
    (FormData.Border in [bsSingle, bsSizeable]);
end;
...
begin
  ...
  begin
    ...
    if (AButton = mbRight) and not IsBorderIconMouseEvent(P, CP, False) and
      (GetWindowLong(FormData.Handle, GWL_STYLE) and (WS_SYSMENU or WS_CHILD) = WS_SYSMENU) then
    begin
      Result := True;
