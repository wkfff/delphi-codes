...
uses
  ...
  cxAccessibility, dxBar, dxBarAccessibility;
...
type
  ...
  protected
    ...
    function GetScreenTip: TdxBarScreenTip; override;
...
type
  ...
  private
    function GetItemControl: TdxBarExtraPanePinnedButtonControl;
  protected
    function HandleNavigationKey(var AKey: Word): Boolean; override;
    procedure Select(ASetFocus: Boolean); override;
    procedure Unselect(ANextSelectedObject: IdxBarAccessibilityHelper); override;
  public
    property ItemControl: TdxBarExtraPanePinnedButtonControl read GetItemControl;
  end;
...
begin
  ...
  Result := cxScreenCanvas.TextHeight(dxMeasurePattern) +
...
function TdxBarExtraPaneControlAccessibilityHelper.GetNextAccessibleObject(
  AItemControl: TdxBarItemControl; ADirection: TcxAccessibilityNavigationDirection): IdxBarAccessibilityHelper;
  ...
  begin
    ...
    try
      GetChildrenForNavigation(AItemControl.IAccessibilityHelper.GetBarHelper,
        Self, GetScreenBoundsForNavigation, ADirection, True, AObjects);
...
function TdxBarExtraPanePinnedButtonControlAccessibilityHelper.HandleNavigationKey(var AKey: Word): Boolean;
  ...
  begin
    Result := (ItemControl.HotPartIndex <> icpNone) and
      (ItemControl.HotPartIndex + ADirection >= 0) and
      (ItemControl.HotPartIndex + ADirection < ItemControl.GetPartCount);
    if Result then
      ItemControl.HotPartIndex := ItemControl.HotPartIndex + ADirection;
  end;
...
begin
  ...
  if ItemControl.HotPartIndex = icpNone then
    ItemControl.HotPartIndex := epbcpButton;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      ABarHelper.ItemControl.HotPartIndex := ItemControl.HotPartIndex;
    end;
  end;
  ItemControl.HotPartIndex := icpNone;
  inherited Unselect(ANextSelectedObject);
end;

function TdxBarExtraPanePinnedButtonControlAccessibilityHelper.GetItemControl: TdxBarExtraPanePinnedButtonControl;
begin
  Result := TdxBarExtraPanePinnedButtonControl(inherited ItemControl);
end;
...
function TdxBarApplicationMenuButtonItem.GetScreenTip: TdxBarScreenTip;
begin
  if FExternalItem <> nil then
