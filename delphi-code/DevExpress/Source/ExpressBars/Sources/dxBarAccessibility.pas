...
begin
  ...
  begin
    ANextBarControl := TdxBarControlAccess(BarControlInstance).GetNextBarControl(not(ssShift in AShift));
    ...
    Result := TdxBarAccessibilityHelperAccess(Childs[AChildIndex]).GetNextAccessibleObject(
      NavigationDirection[BarControlInstance.IsRealVertical, ssShift in AShift]).GetBarHelper;
end;
...
begin
  Result := FOwnerObject as TdxBarSubMenuControl;
end;
