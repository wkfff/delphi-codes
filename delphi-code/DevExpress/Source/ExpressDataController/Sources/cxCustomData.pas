...
uses
  SysUtils, Classes, Variants, cxVariants, cxDataUtils, cxDataStorage, cxFilter, dxCore;
...
//    FDataChangeInfo: TcxDataChangeInfo;
    ...
    function GetIsRowInfoValid: Boolean; {$IFDEF DELPHI9} inline; {$ENDIF}
  ...
  protected
    ...
    procedure DoFilterRecordList(ARecordList: TList);
...
procedure QuickSortList(ASortList: PPointerList; L, R: Integer; ACompareFunc: TCompareItems); overload;
procedure QuickSortList(ASortList: PPointerList; ACount: Integer; ACompareFunc: TCompareItems); overload;
procedure QuickSortList(AList: TList; ACompareFunc: TCompareItems); overload; {$IFDEF DELPHI9} inline; {$ENDIF}
...
procedure InsertionSort(ASortList: PPointerList; ALast: Integer; ACompareFunc: TCompareItems);
var
  I, J: Integer;
  ATemp: Pointer;
begin;
...
procedure ModifiedQuickSort(ASortList: PPointerList; ARight: Integer; ACompareFunc: TCompareItems);
var
  I, J: Integer;
...
begin;
  ...
  begin;
    ...
    begin;
      ...
      ARight := ARight - J;
      Cardinal(ASortList) := Cardinal(@ASortList[J]);
    end
    else
    begin;
      if J + dxInsertionSortLast < ARight then
        ModifiedQuickSort(Pointer(Cardinal(@ASortList[J])), ARight - J, ACompareFunc);
      ARight := I;
    end;
  end;
end;

procedure QuickSortList(AList: TList; ACompareFunc: TCompareItems);
begin;
  QuickSortList(AList.List, AList.Count, ACompareFunc);
end;

procedure QuickSortList(ASortList: PPointerList; L, R: Integer; ACompareFunc: TCompareItems);
begin;
  ASortList := PPointerList(@ASortList[L]);
  QuickSortList(ASortList, R - L + 1, ACompareFunc);
end;

procedure QuickSortList(ASortList: PPointerList; ACount: Integer; ACompareFunc: TCompareItems);
begin;
...
procedure ListSetCount(AList: TList; ANewCount: Integer); {$IFDEF DELPHI9} inline; {$ENDIF}
begin
...
begin
  ...
  DataController := ADataController;
  DoSave;
  QuickSortList(Keys, CompareKeys);
end;
...
begin
  QuickSortList(FItems, CompareSelections);
end;
  ...
  begin
    ...
    try
      if DataController.IsProviderMode and
        ...
        QuickSortList(FRecordList, CompareRecords);
    finally
      UnprepareSorting;
    end;
  end;
...
procedure TcxCustomDataController.DoFilterRecordList(ARecordList: TList);
const
  cxDeletedRecord = Pointer(-1);
var
  I, APrevRecordCount, ACount, ARecordIndex, AEditingRecordIndex: Integer;
  AIsIncrementalFilter, AIsFiltering, AIsOnFilterRecord: Boolean;
begin
  AIsIncrementalFilter := IsIncrementalFilter;
  AIsFiltering := Filter.IsFiltering;
  AIsOnFilterRecord := not IsGridMode and IsFilterRecord;
  if not (AIsIncrementalFilter or AIsFiltering or AIsOnFilterRecord) then
    Exit;

  APrevRecordCount := ARecordList.Count;
  ACount := APrevRecordCount;
  try
    AEditingRecordIndex := Provider.FEditingRecordIndex;
    for I := 0 to ACount - 1 do
      if (I <> AEditingRecordIndex) and
        ( 
         (AIsFiltering and not Filter.DoFilterRecord(I)) or
         (AIsOnFilterRecord and not DoFilterRecord(I)) or
         (AIsIncrementalFilter and not DoIncrementalFilterRecord(I))
        ) then
        ARecordList.List[I] := cxDeletedRecord;
  finally
    ARecordIndex := 0;
    for I := 0 to APrevRecordCount - 1 do
      if ARecordList.List[I] = cxDeletedRecord then
        Dec(ACount)
      else
      begin
        ARecordList.List[ARecordIndex] := Pointer(I);
        Inc(ARecordIndex);
      end;
    ARecordList.Count:= ACount;
  end;

  if ACount <> APrevRecordCount then
  begin
...
begin
  ...
  QuickSortList(AList, CompareIntegers);
...
begin
  Result := (Provider.FEditingRecordIndex <> cxNullEditingRecordIndex) and  
