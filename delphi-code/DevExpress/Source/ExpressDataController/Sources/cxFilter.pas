...
uses
  Classes, dxCore{$IFDEF DELPHI6}, Variants{$ENDIF};
...
begin
  Date1 := GetStartDateOfWeek(Date) - 2 * 7;
  Date2 := Date1 + 2 * 7;
end;
...
begin
  Date1 := GetStartDateOfWeek(Date) + 7;
  Date2 := Date1 + 2 * 7;
end;
...
begin
  ...
  if not Result and FIsUnsortedValues then
    ...
    begin
      Result := CompareItem(I, AValue, ADisplayText) = 0;
      if Result then
      begin
        AIndex := I;
        Break;
...
begin
  Result := PcxFilterValueItem(FItems[Index]);
end;
