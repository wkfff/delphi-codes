...
procedure DialogApplyFont(ADialog: TCustomForm; AFont: TFont);
  ...
  begin
    Result := ADialog.Canvas.TextHeight(dxMeasurePattern);
  end;
...
var
  ...
  ASaveIndex: Integer;
begin
  ADrawToMemory := TMessage(Message).wParam = TMessage(Message).lParam;
  if HasBackground or ADrawToMemory then
  begin
    if not DoubleBuffered or ADrawToMemory then
    begin
...
begin
  if cxIsVCLThemesEnabled and Assigned(Parent) and (csParentBackground in ControlStyle) then
  begin
    cxPaintCanvas.BeginPaint(DC);
    try
      cxDrawThemeParentBackground(Self, cxPaintCanvas, ClientRect);
    finally
