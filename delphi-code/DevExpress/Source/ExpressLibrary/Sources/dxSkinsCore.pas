...
const
  ...
  dxSkinStreamVersion: TdxSkinVersion = 1.04;
  dxSkinImageExt = '.png';
...
type
  ...
  protected
    procedure DataRead(AStream: TStream; const AVersion: Double); override;
  public
  ...
  protected
    ...
    procedure ReadData(AStream: TStream; const AVersion: TdxSkinVersion);
...
procedure TdxSkinColors.DataRead(AStream: TStream; const AVersion: TdxSkinVersion);
var
  I: Integer;
begin
  BeginUpdate;
...
begin
  ...
  FDetails.Name := AName;
  FDetails.OnChanged := SubItemHandler;
...
begin
  if not Empty then
  begin
    Texture.SaveToFile(ChangeFileExt(AFileName, dxSkinImageExt));
    DoChange;
  end;
end;
...
procedure TdxSkinImage.ReadData(AStream: TStream; const AVersion: TdxSkinVersion);
begin
...
procedure TdxSkinElement.ReadData(AStream: TStream; const AVersion: TdxSkinVersion);
var
  ASide: TcxBorder;
begin
  ...
  Glyph.ReadData(AStream, AVersion);
  Image.ReadData(AStream, AVersion);
  for ASide := Low(TcxBorder) to High(TcxBorder) do
  begin
    ReadStringFromStream(AStream); // ClassName
...
procedure TdxSkinElement.WriteData(AStream: TStream);
var
  ASide: TcxBorder;
begin
...
begin
  ...
  try
    ...
    if AVersion >= 1.04 then
    begin
      FDisplayName := ReadStringFromStream(AStream);
      FGroupName := ReadStringFromStream(AStream);
      FNotes := ReadWideStringFromStream(AStream);
...
begin
  WriteStringToStream(AStream, Name);
  WriteStringToStream(AStream, DisplayName);
  WriteStringToStream(AStream, GroupName);
  WriteWideStringToStream(AStream, Notes);
