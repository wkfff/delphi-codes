...
type
  ...
  public
    ...
    function GetAvailablePainter(ANeededThemedObjectType:
      TdxThemedObjectType): TcxCustomLookAndFeelPainter; overload;
    function GetAvailablePainter(ANeededThemedObjectTypes:
      TdxThemedObjectTypes = []): TcxCustomLookAndFeelPainter; overload;
    procedure Refresh;
    procedure Reset;
...
begin
  if not FIsDestruction then
  begin
    if (AChangedValues <> []) or (FCurrentState.UseSkins <> cxUseSkins) then
    begin
      InitializeCurrentState;
      InitializePainter;
      if cxLookAndFeelPaintersManager <> nil then
...
function TcxLookAndFeel.GetAvailablePainter(
  ANeededThemedObjectType: TdxThemedObjectType): TcxCustomLookAndFeelPainter;
begin
  Result := GetAvailablePainter([ANeededThemedObjectType]);
end;

function TcxLookAndFeel.GetAvailablePainter(
  ANeededThemedObjectTypes: TdxThemedObjectTypes = []): TcxCustomLookAndFeelPainter;
begin
  if cxLookAndFeelPaintersManager = nil then
    Result := nil
  else
...
procedure TcxLookAndFeel.PainterChanged(APainter: TcxCustomLookAndFeelPainter);
begin
  Changed(LookAndFeelValueAll);
end;
...
begin
  ...
  SkinName := cxDefaultLookAndFeelSkinName;
end;
