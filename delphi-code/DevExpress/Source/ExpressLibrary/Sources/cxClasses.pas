...
uses
  Windows, Messages, ShellAPI, TypInfo, SysUtils, Classes, Forms, Contnrs,
  SyncObjs, dxCore, dxMessages;
...
uses
  Math, Graphics, Controls, cxGraphics, cxControls;
...
begin
  ...
  begin
    ...
    if IsUnique(AOwnerForm, Result) then
      Break;
  end;
end;
...
begin
  ...
  {$ELSE}
    repeat
      if Result = WAIT_OBJECT_0 + 1 then
        PeekMessage(AMsg, 0, 0, 0, PM_NOREMOVE);
      Result := MsgWaitForMultipleObjects(1, AHandles, False, 1000, QS_SENDMESSAGE);
      if Result = WAIT_FAILED then
        Break;
      if Result = WAIT_OBJECT_0 + 1 then
        CheckSynchronize;
