...
uses
  ...
  dxThemeManager, cxGraphics, cxClasses, cxLookAndFeels, cxLookAndFeelPainters;
...
type
  ...
  private
    ...
    function GetPartState(APart: TcxScrollBarPart): TcxButtonState;
  ...
  protected
    ...
    procedure DrawScrollBarPart(ACanvas: TcxCanvas; const R: TRect;
      APart: TcxScrollBarPart; AState: TcxButtonState); overload; virtual;
    procedure DrawScrollBarPart(ACanvas: TcxCanvas; const R: TRect;
      APart: TcxScrollBarPart); overload; virtual;
    ...
    property PartState[Part: TcxScrollBarPart]: TcxButtonState read GetPartState;
...
uses
  Consts, dxUxTheme, dxThemeConsts, cxControls, cxGeometry;
...
begin
  DrawScrollBarBackground(ACanvas, Bounds(0, 0, Width, Height));
...
procedure TcxScrollBar.DrawScrollBarBackground(ACanvas: TcxCanvas; const R: TRect);
begin
  Painter.DrawScrollBarBackground(ACanvas, R, Kind = sbHorizontal);
end;
...
procedure TcxScrollBar.DrawScrollBarPart(ACanvas: TcxCanvas; const R: TRect;
  APart: TcxScrollBarPart);
begin
  DrawScrollBarPart(ACanvas, R, APart, PartState[APart]);
end;

function TcxScrollBar.GetPartState(APart: TcxScrollBarPart): TcxButtonState;
const
  PartHotStateMap: array[Boolean] of TcxButtonState = (cxbsHot, cxbsNormal);
...
begin
  ...
  begin
    Result := cxbsNormal;
    if (APart <> sbpThumbnail) or ((APart = sbpThumbnail) and IsButtonHotTrack) then
    begin
      if APart = FState.HotPart then
      begin
        if FState.PressedPart = sbpNone then
          Result := PartHotStateMap[csDesigning in ComponentState]
        else
          Result := PartPressedStateMap[APart = FState.PressedPart];
      end;
    end;
  end;
end;
