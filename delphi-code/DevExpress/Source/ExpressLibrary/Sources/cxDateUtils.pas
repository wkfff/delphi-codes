...
function cxGetLocalFirstWeekOfYear: TcxFirstWeekOfYear;
...
function cxGetLocalDateSeparator: Char;
begin
  Result := dxGetLocaleInfo(GetThreadLocale, LOCALE_SDATE, '')[1];
end;
...
begin
  ...
  begin
    ...
    if I > 7 then
      Dec(I, 7);
    Result := dxFormatSettings.ShortDayNames[I];
  end
  else
    Result := dxGetLocaleInfo(GetThreadLocale, cxDayNameLCType[IsWinVistaOrLater, I]);
...
procedure CorrectTextForDateTimeConversion(var AText: string;
  ...
  begin
    ...
    begin
      AList.Add(cxGetLocalDateSeparator);
      AList.Add(cxGetLocalTimeSeparator);
      AList.Add(cxGetLocalTimeAMString);
      AList.Add(cxGetLocalTimePMString);
    end
    else
    begin
...
begin
  ...
  if AOleConversion then
    InternalStringReplace(S, cxGetLocalDateSeparator);
  AText := S;
end;
...
function GetWeekNumber(ADate: TDateTime; AStartOfWeek: TDay;
  ...
  begin
    AYearStart := EncodeDate(AYear, 1, 1);
    case AFirstWeekOfYear of
      fwyJan1:
        Result := FindFirstDayOfWeekDate(AYearStart);
      fwyFirstFourDays:
        Result := FindFirstDayOfWeekDate(AYearStart + 3);
      else //fwyFirstFullWeek
        Result := FindFirstDayOfWeekDate(AYearStart + 6);
    end;
  end;
...
begin
  if AFirstWeekOfYear = fwySystem then
    AFirstWeekOfYear := cxGetLocalFirstWeekOfYear;
  DecodeDate(ADate, AYear, AMonth, ADay);
  if (AMonth = 12) and (ADate >= GetYearFirstWeekDate(AYear + 1)) then
    Result := 1
  else
  begin
...
function cxDateTimeToText(ADate: TDateTime; AFourDigitYearNeeded: Boolean = False; AUseDelphiDateTimeFormats: Boolean = False): string;
  ...
  begin
    ...
    begin
      ...
      Result := cxGetLocalShortDateFormat;
...
initialization
  FCalendarList := TcxObjectList.Create;
