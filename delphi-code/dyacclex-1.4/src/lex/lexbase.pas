...
function key(symbol: string; table_size: integer;
...
function definedKey(symbol: string; table_size: integer;
...
function charStr(c: char; reserved: CClass): string;
  ...
  begin
    Result := intStr(Ord(c) div 64) + intStr((Ord(c) mod 64) div 8) +
      intStr(Ord(c) mod 8);
  end(*octStr*);
begin
  case c of
...
function regExprStr(r: RegExpr): string;
  ...
  begin
    if is_epsExpr(r) then
      ...
      Result := '#(' + intStr(rule_no) + ',' + intStr(pos) + ')'
