{
  ...
  ## $Id: yacctabl.pas 1325 2004-08-17 20:07:24Z druid $
...
var
  act_state: integer; (* state currently considered *)
...
procedure include_item_set(var item_set: ItemSet; rule_no, pos_no: integer);
...
begin
  Inc(n_nts);
  if n_nts > max_nts then
    fatal(nt_table_overflow);
  sym_type^[ -n_nts] := 0;
  Result := -n_nts;
end(*new_nt*);
...
begin
  Inc(n_lits);
  if n_lits > max_lits then
    fatal(lit_table_overflow);
  sym_type^[n_lits - 1] := 0;
  sym_prec^[n_lits - 1] := 0;
  Result := n_lits - 1;
end(*new_lit*);
...
function get_key(symbol: string): integer;
begin
  Result := key(symbol, max_keys, {$ifdef fpc}@
{$endif}
    lookup,
                           {$ifdef fpc}@
{$endif}
...
begin
  case sym of
    1..255: Result := singleQuoteStr(chr(sym));
    0: Result      := '$end';
    -1: Result     := '$accept';
    else
    begin
      if sym_table^[sym_key^[sym]].pname^[1] = '''' then
      begin
        Result := singleQuoteStr(
        ...
        Result := sym_table^[sym_key^[sym]].pname^;
      end;
    end;
  end;
end(*pname*);
...
begin
  if rule_table^[rule_no^[i]]^.lhs_sym =
    ...
    Result := rule_no^[i] < rule_no^[j]
  else
    Result := rule_table^[rule_no^[i]]^.lhs_sym >
...
begin
  Inc(n_types);
  if n_types > max_types then
    fatal(type_table_overflow);
...
{$ifndef fpc}{$F-}{$endif}
begin
  Result := sym_table^[type_table^[i]].pname^ <
...
begin
  ...
  Result := (k <= n_types) and (sym_table^[type_table^[k]].pname^ = symbol);
end(*search_type*);
...
begin
  Inc(n_prec);
  if n_prec > max_prec then
    fatal(prec_table_overflow);
...
begin
  Inc(n_trans);
  if n_trans > max_trans then
    fatal(trans_table_overflow);
...
begin
  Inc(n_redns);
  if n_redns > max_redns then
    fatal(redn_table_overflow);
...
begin
  with state_table^[s] do
    Result := item_hi - item_lo + 1
end(*n_state_items*);

function n_state_trans(s: integer): integer;
begin
  with state_table^[s] do
    Result := trans_hi - trans_lo + 1
end(*n_state_trans*);

function n_state_redns(s: integer): integer;
begin
  with state_table^[s] do
    Result := redns_hi - redns_lo + 1
end(*n_state_redns*);
...
begin
  with sort_items do
    if item[i].pos_no = item[j].pos_no then
      Result := item[i].rule_no < item[j].rule_no
    else
      Result := item[i].pos_no > item[j].pos_no
end(*items_less*);
...
begin
  sort_items := item_set;
  quicksort(1, sort_items.n_items, {$ifdef fpc}@
{$endif}
    items_less,
                  {$ifdef fpc}@
{$endif}
...
begin
  ...
  for i := -max_nts to max_lits - 1 do
    sym_type^[i] := 0;
  for i := 0 to max_lits - 1 do
    sym_prec^[i] := 0;
  for i := 1 to max_rules do
    rule_prec^[i] := 0;
