{
  ...
  ## $Id: yaccpars.pas 1325 2004-08-17 20:07:24Z druid $
...
var
  shift_reduce, reduce_reduce, never_reduced: integer;
...
var
  reduced: array [1..max_rules] of boolean;

var
  yynacts, yyngotos, yynstates: integer;
...
var
  ...
  i, j, k, s, n_errors, n_shifts, n_gotos, n_reductions,
  n_conflicts: integer;
    ...
    begin
      ...
      for i := trans_lo to trans_hi do
        with trans_table^[i] do
          if sym >= 0 then
            for j := redns_lo to redns_hi do
              with redn_table^[j] do
...
begin
  ...
  begin
    ...
    begin
      Write(yyout, '{ ' + IntToStr(i) + ': } ( tokenname: ' +
        AnsiQuotedStr(pname(i), '''') + ' )');
      if i < n_lits - 1 then
        Write(yyout, ',');
      writeln(yyout);
    end;
